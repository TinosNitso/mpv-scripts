...
if o.scripts then for _,script in pairs(o.scripts) do local is_present,CscriptC,scripts = false,'"'..script..'"',mp.get_property('scripts')  --CscriptC="script" ("'" & "," MAY BE IN FILENAMES). TRY LOAD ALL scripts IF NOT ALREADY LOADED.
        local SCRIPTSU,SCRIPTU = scripts:upper(),script:upper()    --SEARCH NOT CASE SENSITIVE. CHECK IF ALREADY LAUNCHED.
...
----UNLIKE A PLUGIN THE ONLY BINARY IS MPV ITSELF, & THE SCRIPTS COMMAND IT. MOVING MASK, SPECTRUM & audio RANDOMIZATION ARE JUST MPV COMMANDS. HOWEVER THE FEWER COMMANDS THE BETTER (1 command PER SMOOTH-CROP, ETC). E.G. EVERY MASK INVERSION COULD BE A vf-command, BUT IT'S BETTER TO LET IT INVERT ITSELF, IN AUTO.
