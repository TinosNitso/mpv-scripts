...
----WORKS WITH JPG, PNG, GIF, BMP, MP3 COVER ART, MP4 ETC, IN SMPLAYER & MPV (DRAG & DROP). .TIFF ONLY DISPLAYS 1 LAYER (BUG). NO WEBP, PDF OR TXT. WORKS WELL WITH YOUTUBE (ytdl). LOAD TIME SLOW DUE TO LACK OF BUFFERING. FULLY RE-BUILDS ON EVERY seek. WITHOUT autocomplex (JPEG ANIME) A STILL FRAME USES 0% CPU. CHANGING vid TRACKS (MP3TAG) SUPPORTED. NO FANCY TITLE (drawtext) OR CLOCK IN THIS SCRIPT. SCRIPT IMPOSSIBLE TO READ/EDIT WITH WORD WRAP, WHICH MAY BE A PROBLEM ON MACOS (A BIBLE HAS NO SCROLLBAR). 
...
options={ --ALL OPTIONAL & MAY BE REMOVED (FOR SIMPLE NEGATIVE).      nil & false → DEFAULT VALUES    (BUT BLANK string ''=true).
    ...
    filters='null,' --CAN REPLACE null WITH OTHER FILTERS, LIKE pp (POSTPROCESSING). TIMELINE SWITCHES ALSO POSSIBLE (FILTER1→FILTER2→ETC).
          ..'lutyuv=maxval*((negval/maxval)^4*(1+.6*.2)+.2*(1.5*gauss((255-val)/(255-maxval)/2-1.5)-1*gauss(val/minval/1.5-1.5))/gauss(0)+.01*sin(2*PI*val/minval))'   --+2% SINE WAVE, MAY HELP DIFFERENTIATE @THE BLACKEST BLACK. period=minval
                 ..':128*(1+(val/128-1)/abs(val/128.01-1)^.2)' --u & v USE A POWER LAW NON-LINEAR SATURATION. sqrt ("^.5") TOO POWERFUL, SO CAN DIVIDE BY "^.2". GREYSCALE IS CENTERED ON 128 NOT 127.5 (128 MEANS 0 IN POPULAR YUV COVERSION FORMULAS). TOO MUCH & HUMAN FACES MAY LOOK REDDISH-BLACK.
                 ..':128*(1+(val/128-1)/abs(val/128.01-1)^.2)', --sgn FUNCTION NOT LINUX snap COMPATIBLE, HENCE 128.01 ISSUE. MOVES PERCENTAGE saturation INTO NON-LINEAR EXPONENT (uv INSTEAD OF rgb).
    ...
    -- SQUARE=true,--UNCOMMENT FOR RECTANGULAR SECTIONS, INSTEAD OF DISCS.    IT'S ALSO POSSIBLE TO EASILY ADD SWITCHES FOR CUT DIAGONALS & FAT-CIRCLE (HALFWAY BTWN SQUARE & CIRCLE). MORE SWITCHES COMPLICATES THE SCRIPT (& options). DEFAULT IS CIRCLE EYES FOR RECTANGLE HEAD.
    -- blur=0,     --DEFAULT=1, OR 0. IRRELEVANT IF SQUARE. UNCOMMENT FOR CUT-CORNERS. (WITHOUT avgblur THERE'S NOTHING CIRCULAR.)
    -- ROUND_SQUARE=true,  --FOR ROUNDED SQUARES INSTEAD, UNLESS SQUARE OR blur=0.
...
for key,val in pairs({filters='lutyuv=negval',fps=25,periods=1,DUAL=1,INVERT='0',DISABLE='0',blur=1,x='0',y='0',rotations='0',zoompan='1:0:0',RES_MULT=1,RES_SAFETY=1,LEAD_T=0,periods_skipped=0,format='yuv420p',scale={},key_bindings='',toggle_on_double_mute=0,io_write='',config={}})
...
p=o.period --ABBREV.
if o.SECTIONS==0 and o.periods==0 then NULL_OVERRIDE=true end
...
N,rot_enable  = 0,1    --N>=0 IS SECTION #. enable (1 OR 0) IS FOR avgblur ROTATIONS (FAT CIRCLE → CIRCLE).

if o.ROUND_SQUARE then rot_enable=0 end
if not o.SQUARE then DISC=(',avgblur=enable=%d,scale=iw*2:-1:bicubic,split[1],rotate=PI/4:enable=%d[2],[1][2]lut2=x*y/255,split[1],rotate=PI/8:enable=%d[2],[1][2]lut2=x*y/255'):format(o.blur,rot_enable,rot_enable) end  --DRAW CIRCLE BEFORE MASK. MAY USE INTERMEDIATE scale FOR 22.5° & 45° ROTATIONS. A THIRD PI/16 ROTATION MAY BE USED FOR SMOOTHER MONACLE.
...
if NULL_OVERRIDE then lavfi=o.filters end   --FAST LOAD


function start_file()   --EMBEDDED MPV PLAYLISTS REQUIRE INSTA-pause BEFORE GRAPH INSERTION.    'set geometry' ISN'T THE REASON.
...
function file_loaded()  --ALSO STREAM.
    ...
    brightness,fps = -1,'fps='..o.fps   --LEAD FRAME OF FILM SHOULDN'T BE MASKED (JPEG OPPOSITE).
    ...
    if NULL_OVERRIDE or fps=='null' and mp.get_property('lavfi-complex')=='' then is1frame=true end   --USE FULL toggle: "vf toggle" NOT "vf-command".
end
mp.register_event('file-loaded',file_loaded)

function playback_restart()   --vf-command FOR NORMAL MP4.
    mp.command(('vf-command %s brightness %s'):format(label,brightness)) 
    if OFF or is1frame then return end
...
function on_toggle(mute) 
    ...
    if is1frame then mp.command('no-osd vf toggle @'..label)  --NULL_OVERRIDE & MP3 (NO complex): VERIFY USING "F2" TOGGLE, OR DOUBLE-mute. ALSO WORKS IN TANDEM WITH autocrop. UNFORTUNATELY MPV SNAPS WHEN EMBEDDED. (A REPLACEMENT SNAP GRAPH WOULD REQUIRE AN EXTRA LINE OF CODE.)
