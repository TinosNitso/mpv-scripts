...
options={  --ALL OPTIONAL & MAY BE REMOVED (FOR SIMPLE NEGATIVE).      nil & false → DEFAULT VALUES    (BUT '' MEANS true). 
    ...
    -- MONACLE        , SECTIONS=1,DUAL=nil,widths=nil,heights='ih',x=nil,y=nil,crops=nil,rotations=nil,zoompan=nil, --INVERTING MONACLE.  geq=nil  FOR SQUARE.  SECTIONS>1 FOR CONCENTRIC DISCS.  EXACT POSITIONING IS TRIAL & ERROR.  MONACLE MAY BE THE BEST OVERALL.
    -- BINACLES       , SECTIONS=1,widths='iw',heights=nil,x=nil,y=nil,crops=nil,zoompan=nil,
    -- PENTAGON_HOUSE , SECTIONS=1,DUAL=nil,widths=nil,heights='ih+2',x=nil,y=nil,crops=nil,rotations=nil,zoompan=nil,geq='255*lt(abs(X-W/2)\\,Y)',  --+2 TO REACH THE BOTTOM.  THE LENS LIVES INSIDE A HOUSE!  IT MAY BE POSSIBLE TO EXTEND USING HALF-LUMINANCE.
    ...
    -- TRIANGLE_SPIN  , SECTIONS=1,DUAL=nil,widths=nil,heights='ih',x=nil,y=nil,crops=nil,rotations='2*PI*n/%s/3',zoompan=nil,geq='255*lt(Y\\,H*3/4)*lt(abs(X-W/2)\\,Y/sqrt(3))',  --SPINNING EQUILATERAL TRIANGLE, GRAZING TOP & BOTTOM OF SCREEN.  HYPOTENUSE=87% OF HEIGHT (sqrt(3)/2) OF FULLSCREEN DISPLAY.  GOOD FOR TESTING automask2.lua.
    -- DIAMOND_EYES   , geq='255*lt(abs(X-W/2)+abs(Y-H/2)\\,W/2)',
    -- TWIRL_SKIP     , periods =3,periods_skipped=1,negate_enable='gte(n\\,%s)',y='-H*((c)/16+1/16) H/16 H/32*(s) H/32*((c)+1)/2 H/64',zoompan=nil,  --DOUBLE-TWIRL & SKIP 
...
----geq             = lum   DEFAULT=lum(X\\,Y)  GLOBAL EQUALIZER IS SLOW, EXCEPT ON 1 FRAME. CAN DRAW ANY SHAPE.  EVEN IN A *NON*-PERIODIC DESIGN ITS DRAWING/S CAN BE RECYCLED INDEFINITELY.  st,ld = STORE,LOAD  FUNCTIONS MAY IMPROVE PERFORMANCE.  IT CAN ALSO ACT SMOOTHLY ON TINY VIDEO.  THE FORMULA FOR REGULAR PENTAGON MAY BE LIKE 5 GRADIENTS DETERMINED BY atan. SPIKED EYES MAY BE POSSIBLE.
...
m,brightness,label = {},0,mp.get_script_name() --MEMORY FOR vid & brightness. label=automask  par ASSUMED 1 UNTIL OBSERVATION.
...
function file_loaded() --ALSO @seek, @vid, @on_toggle(is1frame) & @osd-par.
    ...
    elseif not (is1frame and brightness==-1)       --CHECK NOT ALREADY DISABLED is1frame.
