----AUTO ANIMATED MASK GENERATOR & MERGE SCRIPT, FOR VIDEO & IMAGES, IN MPV & SMPLAYER, WITH INSTANT DOUBLE-mute TOGGLE (m&m FOR MASK). IF PAUSED INSTA-TOGGLE FRAME-STEPS TOO. COMES WITH 10 MORE EXAMPLES INCLUDING MONACLE, BINACLES, VISORS, ETC. MOVING POSITIONS, ROTATIONS & ZOOM. THE SAME GENERATOR CODE GENERATES MANY DIFFERENT MASKS, WITHOUT BEING AS SPECIFIC AS A .GIF (A GIF IS MORE EXPENSIVE). USERS CAN FREELY COPY/PASTE PLAIN TEXT INSTEAD.
----APPLIES ANY LIST OF ffmpeg-filters TO MASKED REGION, WITH INVERSION & INVISIBILITY. FILTERS HELP DETECT DEFECTS, LIKE HAIR ON PASSPORT SCAN. FULLY PERIODIC FOR BEST RES & PERFORMANCE. IT'S LIKE OPTOMETRY FOR TELEVISION, BUT SOME MASKS ARE PURELY DECORATIVE. A TV CAN ONLY BE AS GOOD AS THE ANIME ON IT! DILATING PUPILS NOT CURRENTLY SUPPORTED - EASY TO CODE BUT WOULD BREAK LINUX snap COMPATIBILITY. 
----WORKS WITH JPG, PNG, GIF, BMP, MP3 COVER ART, MP4 ETC, IN SMPLAYER & MPV (DRAG & DROP). .TIFF ONLY DISPLAYS 1 LAYER (BUG). NO WEBP, PDF OR TXT. LOAD TIME SLOW DUE TO LACK OF BUFFERING. WITHOUT autocomplex (JPEG ANIME) A STILL FRAME USES 0% CPU. CHANGING vid TRACKS (MP3TAG) SUPPORTED. NO FANCY TITLE (drawtext) OR CLOCK IN THIS SCRIPT. SCRIPT IMPOSSIBLE TO READ/EDIT WITH WORD WRAP, WHICH MAY BE A PROBLEM ON MACOS (A BIBLE HAS NO SCROLLBAR). 
...
options={ --ALL OPTIONAL & MAY BE REMOVED (FOR SIMPLE NEGATIVE).      nil & false → DEFAULT VALUES    (BUT BLANK string ''=true).
    FILTERS='lutyuv=minval+(maxval-minval)*((negval/maxval)^4*(1+1/5)+1/5*(2*gauss((255-val)/(255-maxval)/1.5-1.5)-1*gauss(val/minval/1.5-1.5))/gauss(0))' 
         ...
         ..',null',                                               --CAN REPLACE null WITH OTHER FILTERS, LIKE pp (POSTPROCESSING). TIMELINE SWITCH enable ALSO (FILTER1→FILTER2→ETC) POSSIBLE.
    ...
    -- SQUARE=true,--FOR RECTANGULAR SECTIONS, INSTEAD OF DISCS.    IT'S ALSO POSSIBLE TO EASILY ADD SWITCHES FOR CUT DIAGONALS & FAT-CIRCLE (HALFWAY BTWN SQUARE & CIRCLE). MORE SWITCHES COMPLICATES THE SCRIPT (& options).
    ...
    x='-W/64*(s)       0    W/16*(1+(c)) W/32*(c)       W/64', --(c),(s) = (cos),(sin) WAVES IN TIME=t/period. overlay COORDS FROM CENTER. W IS THE BIGGER DISC/CANVAS, & w IS THE DISC.  (t)=(time) (n)=(frame#) (p)→(period) (c)→(cos(2*PI*(t)/(p))) (s)→(sin(2*PI*(t)/(p))) (T)→((t)/(p))   %s=period=(p)        NB: FUTURE VERSION SHOULD HAVE 0TH LEVEL COORDS FOR COMBINED DUAL.
    ...
    zoompan  ='1+.2*(1-cos(2*PI*((in)/%s-.2)))*mod(floor((in)/%s-.2)\\,2):0:0',--%s=FRAMES-PER-PERIOD=fps*period  (zoom:x:y)  in=INPUTFRAME#=on=OUTPUTNUMBER  mod IS AN ON/OFF SWITCH.     20% zoom FOR RIGHT PUPIL TO PASS SCREEN EDGE. 20% PHASE OFFSET, LIKE A BASEBALL BAT'S ROTATIVE WIND UP.
    ...
    toggle_on_double_mute=.5,  --DEFAULT=0 SECONDS (NO TOGGLE). TIMEOUT FOR DOUBLE-mute TOGGLE. ALL LUA SCRIPTS CAN BE TOGGLED BY DOUBLE mute.
    ...
    config  ={
              ...
              'pause yes',  --THEN UNPAUSE @file-loaded. EMBEDDED MPV MAY SNAP IF IT ISN'T PAUSED BEFORE GRAPH'S INSERTED.    autocrop SHOULD ALSO UNPAUSE AFTER GRAPH INSERTION, SO IMPOSSIBLE TO START PAUSED (IRONICALLY).
             },       
}
...
for key,val in pairs({FILTERS='lutyuv=negval',fps=25,periods=1,DUAL=1,INVERT='0',DISABLE='0',x='0',y='0',rotations='0',zoompan='1:0:0',RES_MULT=1,RES_SAFETY=1,LEAD_T=0,periods_skipped=0,scale={},key_bindings='',toggle_on_double_mute=0,io_write='',config={}})
...
blur,N,MASK = '',0,'%s' --blur INITIALIZES CIRCLE FROM SQUARE. N>=0 IS SECTION #. MASK IS GENERATED RECURSIVELY, FROM %s. rotate COUPLES WITH zoompan, AFTER DUAL.
...
while N<o.SECTIONS do N=N+1 --MASK CONSTRUCTION, BEFORE DUAL.
    ...
    NEWDISC,pre_overlay = '',''  --NEWDISC FOR N>1. THEN (POSSIBLY) crop & rotate. 
    ...
    if N>1  then NEWDISC=(',split[%d],scale=floor((%s)/4)*4:floor((%s)/4)*4%%s,lut=255-val'):format(N-1,g.w[N],g.h[N]) end  --split N-1 & scale. %%s IS FOR N=2. FLOORED MULTIPLES OF 4 WORK BEST (2 MAY CAUSE overlay CENTERING BUG). lut WORKS IN LINUX snap, BUT NOT negate (snap DOESN'T SUPPORT ALL FILTERS).  THIS IS WHERE DILATING PUPILS GO, BY ADJUSTING scale.
...
----eq     =...:brightness  DEFAULT 0  RANGE [-1,1]  EQUALIZER GIVES SNAP-FREE INSTA-TOGGLE, DIRECTLY FROM LUA. FILM & JPEG OPPOSITE: FILM MASK OFF BY DEFAULT (EXCEPT BY vf-command).   eq MAY ALSO BE USED IN o.FILTERS, BUT NOT ITS brightness DUE TO TOGGLE INTERFERENCE.
...
function file_loaded()
    ...
    if not (W and H) then mp.add_timeout(.05,file_loaded)   --LINUX FALLBACK: RE-RUN & return FOR DIMENSIONS. DUE TO EXCESSIVE LAG IN VIRTUALBOX.
        return end
    W,H = math.ceil(W/4)*4,math.ceil(H/4)*4 --BUGFIX: MULTIPLES OF 4 NECESSARY FOR PERFECT overlay.
    
    brightness,fps = -1,'fps='..o.fps   --LEAD FRAME OF FILM IS SPECIAL & SHOULDN'T BE MASKED (JPEG OPPOSITE).
    if mp.get_property_bool('current-tracks/video/image') then brightness,fps = 0,'null'   --IF THERE'S A LOOPING complex, THE o.fps SHOULD MATCH, FOR SYNCHRONY.  JPEG (NO loop) SHOULD USE 0% CPU. IMAGES MAY BE JPG, PNG, BMP, MP3. GIF IS not image. NO WEBP. TIFF TOP LAYER ONLY. 
        if o.io_write=='' then o.io_write=' ' end end   --JPEG NEEDS io FIX (CAN VERIFY SNAPS LESS OFTEN).
    mp.observe_property('vf','native',function() io.write(o.io_write) end)
    
    mp.command(('no-osd vf append @%s:lavfi=[%s]'):format(label,lavfi):format(fps,W,H,W,H,brightness))     --INSERT GRAPH. IT GOES AFTER autocrop. W,H REPEAT FOR zoompan.
...
timer=mp.add_periodic_timer(o.toggle_on_double_mute, function()end)    --DOUBLE mute timer DEFINED BEFORE TOGGLE. IT CARRIES OVER TO NEXT video IN MPV PLAYLIST.
timer.oneshot=true 
timer:kill()

function on_toggle(mute)    --INSTA-TOGGLE ONLY - NOT FULL TOGGLE.
    if mute=='mute' and not timer:is_enabled() then timer:resume() --START timer OR ELSE TOGGLE.
        return end  --TOGGLE BELOW.
        
    OFF=not OFF
    if  not OFF then brightness= 0     --ON   
    else             brightness=-1 end --OFF
    vf_command()
    
    if fps=='null' then mp.command('no-osd vf toggle @'..label) end --MP3: VERIFY USING "F2" TOGGLE, OR DOUBLE-mute. ALSO WORKS IN TANDEM WITH autocrop. UNFORTUNATELY SNAPS (A REPLACEMENT ANTI-SNAP GRAPH WOULD REQUIRE AN EXTRA LINE OF CODE).
...
function vf_command() --INSTA-TOGGLE automask DIRECTLY FROM LUA.   IDEAL POSITIONING SYSTEM MAYBE LIKE GAMEPAD→vf-command, BUT THIS automask ISN'T THAT GOOD.
    mp.observe_property('seeking','bool',function() mp.command(('vf-command %s brightness %s'):format(label,brightness)) end)  --TRIGGERS INSTANTLY, & ON seeking BECAUSE THAT RESETS GRAPH STATE.  SIMILAR MECHANISM COULD ALSO INVERT, BUT THE TIMELINE SWITCH IS SUPERIOR.
    if mp.get_property_bool('pause') then N=0   --vf-command BUGFIX FOR PAUSED MP4. frame-step IS FASTER & SIMPLER THAN FULL GRAPH TOGGLE.  pause IS NEEDED FOR CALIBRATING lutyuv. 
        while N<5 do N=N+1  --5 STEPS REQUIRED. video-timing-offset=1 MAY HAVE ALREADY DRAWN FRAMES IN ADVANCE. AN UNPAUSE→pause timer CAN ALSO BE USED.
            mp.command('frame-step') end end    --frame-back-step MAY CAUSE seek. seek INVALID.
end
...
----MASK REQUIRES EXTRA 300MB RAM. E.G.  256MiB = 1680*1052*4*19*2/1024/1024 = display*yuva*19FRAMES*2periods/1KiB/1KiB   
