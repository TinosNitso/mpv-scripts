----lavfi-complex SCRIPT WHICH LIMITS fps & size (TO SCREEN); LOOPS JPEG; & OVERLAYS STEREO FREQUENCY SPECTRUM + volume BARS (AUDIO VISUALS) ONTO MP4, MP3 (RAW & COVER ART), WITH DOUBLE-mute TOGGLE. ACCURATE 100Hz GRID (LEFT 1kHz TO RIGHT 1kHz).     
...
local options={ --ALL OPTIONAL & MAY BE REMOVED. TO REMOVE A COMPONENT SET ITS alpha TO 0 (freqs, volume & feet).
    ...
    -- CALIBRATION={{100,1},{'200:1',2},{300,1},{'400:1',2},{500,1},{'600:1',2},{700,1},{'800:1',2},{900,1},{'1000:1',2}}, --{{frequency(Hz):beep_factor,volume},}.    sine WAVES FOR CALIBRATION.  THIS EXAMPLE BEEPS DOUBLE ON EVEN.   COULD HELP TEST NORMALIZERS, & DECORATE freqs.  SPECTRUM CALIBRATES VIA 3 NUMBERS: freqs_lead freqs_sr LR_OVERLAP   SYNCHRONY IS freqs_lead.
    
    freqs_highpass     =   25, --DEFAULT= 25 Hz. DAMPENS SUB-BASS & DC, IN THE MIDDLE. 10 Hz LETS THROUGH TOO MUCH SUB-BASS.
    ...
    format   ='yuv420p',  --DEFAULT=yuv420p  IMPROVES PERFORMANCE, LIKE LIMITING fps. 420p LIMITS COLOR RESOLUTION TO HALF-WIDTH & HALF-HEIGHT.
    -- scale ={1680,1050},--DEFAULT=display (WINDOWS & MACOS), OR ELSE =video (LINUX). scale OVERRIDE.  USES CLOSEST MULTIPLES OF 4 (ceil).
    
    key_bindings         ='F1',--DEFAULT='' (NO TOGGLE). CASE SENSITIVE. M IS MUTE SO CAN DOUBLE-PRESS M. F=FULLSCREEN NOT FREQS, O=OSD, C=autocrop. V FOR VOLUME? S=SCREENSHOT NOT SPECTRUM. 'F1 F2' FOR 2 KEYS.  key_bindings DON'T WORK INSIDE SMPLAYER.
...
local FP,CALIBRATION,ORIENTATION = o.fps*o.period,'','' --ABBREV. FP FRAMES/PERIOD. W,H scale OVERRIDE. CALIBRATION MIXES IN SINE WAVE LIST. ORIENTATION IS FOR TOP & BOTTOM.
...
----loop             =loop:size  (LOOPS>=-1 : FRAMES/LOOP>0) -1:1 FOR CASE 1: JPEG (NO SPECTRUM). PAIRS WITH fps. JPEG WITH SOUND USES asplit FOR SEEKING, SO RAW JPEG IS ITS OWN CASE WITH ITS OWN FILTER.
...
function file_loaded()
    ...
    scale=('scale=%d:%d,setsar=1,format=%s'):format(W,H,o.format)   --EVERY CASE REPEATS THIS AGAIN & AGAIN.
    
    ----5 CASES. 1) JPEG. 2) MP4 LIMITS. 3) MP3  4) MP3+JPEG  5) MP4     INSTEAD OF EVERY SCRIPT SETTING ITS OWN lavfi-complex, THEY MAY RELY ON THIS SCRIPT. E.G. autocrop JPEG USES MORE CPU IF COMBINED WITH autocomplex, FOR THE BLACK-BAR-STRETCH. JPEG ANIME MAY PRODUCE A STILL FRAME ONLY, WITHOUT THIS SCRIPT.
    ...
    if not aid or ORIENTATION=='' then if not complex then mp.set_property('lavfi-complex','') end  --NULLIFY NULL complex. OTHER SCRIPTS CAN CHECK BACK WITHIN 50ms.
        return end --CASES 3,4,5 BELOW. complex VARIABLE MUST PRODUCE [vo].

    if           not vid then complex=('[vid]split[vid],%s'):format(scale) end    --CASE 3: RAW MP3 DOUBLE-SPECTRUM (STILL & MOVING). SIMPLEST CODE IS TO LITERALLY DOUBLE IT. IT USES ITSELF AS A STEADY PLATFORM ON WHICH TO SCOOT.
    if     image and vid then complex=('[vid%d]%s[vo],[vid]split[vid],crop=1:1:1:1:1:1[C],[C][vo]scale2ref,overlay'):format(vid,scale) end    --CASE 4: MP3 COVER ART.   FOR PROPER MOTION, [vo] GETS SANDWICHED BTWN CANVAS [C] & [vid] (SPECTRUM).
    if not image and vid then complex=('[vid%d]fps=%s,%s'):format(vid,o.fps,scale) end --CASE 5: NORMAL video. LIMIT fps & scale.
    
    mp.set_property('lavfi-complex',lavfi:format(aid,complex,W,H*o.freqs_clip_h*2))    --CASES 3, 4 & 5. aid TO asplit. W,H FOR zoompan (FINAL CLIP HEIGHT FOR PADDED TOP & BOTTOM). mp.set_property IS MORE POWERFUL THAN mp.command (LIKE mp.commandv).
end 
...
function on_toggle(mute)
    ...
    if not OFF then file_loaded()   --TOGGLE ON.    UNFORTUNATELY THIS SNAPS COVER ART IN SMPLAYER.
        ...
        local complex=('[aid%d]anull[ao]'):format(aid)    --CASE 3: RAW audio. STILL FRAME SPECTRUM, audio PLAYS ON. CASES 4 & 5 BUILD ON THIS.
        if             image then complex=('%s,[vid%d]%s,loop=-1:1,fps=%s:%s[vo]'):format(complex,vid,scale,o.fps,mp.get_property_number('time-pos')) end   --CASE 4: COVER ART. NEED start_time FOR ZOOMING automask (CAN TOGGLE "F1" @30MINS). STILL USES APPROX 5% CPU TO INFINITE loop. 
    ...
    mp.observe_property('seeking', 'bool', function() io.write(' ') end)
end
...
----OPTION DUMP. FOR DEBUGGING TRY TOGGLE ALL THESE SIMULTANEOUSLY. SOMETIMES LINES OF CODE CAN BE DELETED WITH THE CORRECT OPTION.
