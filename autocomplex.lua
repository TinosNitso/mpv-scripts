...
o,label   = options,mp.get_script_name()  --label=autocomplex (USED TO FISH FOR ERRORS).
...
----zoompan         = z:x:y:d:s:fps     (z>=1)  DEFAULT=1:0:0:INFINITE:hd720:25  d=1 (OR 0) FRAMES DURATION-OUT PER FRAME-IN.  z:x:y MAY DEPEND ON  INPUT-NUMBER=in=on=OUTPUT-NUMBER  zoompan OPTIMAL FOR ZOOMING.
...
----format,aformat  = pix_fmts,sample_fmts:sample_rates  {yuva420p,yuv420p,bgra,gbrp=rgb24,gbrap},s16:Hz  IS THE FINISH (TO REMOVE alpha). MAY BE BLANK.  gbrap (GreatBRitAin Planar) ASSUMED BY shuffleplanes & avgblur. REQUIRED IN FFMPEG-v4.2.  HOWEVER bgra (RGB BACKWARDS) ALSO REQUIRED FOR EFFICIENT SCALING (SOLVES ERRORS). SO IT'S AN INTERMEDIATE BTWN gbrap & yuva420p, WHICH IS FORCED BY overlay.  aformat REMOVES doublep PRECISION AFTER dynaudnorm, & DOWNSAMPLES TO 2.1kHz (NYQUIST+5%). 
...
function file_loaded()  --ALSO on_aid_vid & on_toggle{ON}.    THIS COULD BE REPLACED BY on_av_params. BUT THE .1s DELAY TO DETECT alpha RARELY CAUSES STUTTER/LAG @LOAD.
    ...
    duration       = p.duration and p.duration-.1  --nil FOR JPEG.  SUBTRACT .1s, BY TRIAL & ERROR. SAFER THAN SETTING end PROPERTY.  TESTED MPV-v0.38 ON 10 HOURS albumart (HANGS NEAR end-file).
