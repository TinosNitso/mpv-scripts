...
options={ --ALL OPTIONAL & MAY BE REMOVED. TO REMOVE A COMPONENT SET ITS alpha TO 0 (freqs, volume, grid & feet).
    ...
    highpass  =100, --DEFAULT=100 Hz  DAMPENS SUB-BASS & DC, NEAR volume BAR. 100 Hz FOR BASS HEAVY TRACKS. highpass & dynaudnorm APPLY ONLY TO VISUALS, NOT [ao] (AUDIO OUTPUT). 
    dynaudnorm='250:11:1:100:0:0:1:1', --DEFAULT=500 ms  f:g:p:m:r:n:c:b  APPLIES TWICE, BEFORE & AFTER freqs_resample & freqs_lead. SEE GRAPH SECTION FOR DETAILS.
    ...
    freqs_resample=2000*1.05,--DEFAULT=2000*1.05 Hz. SAMPLE RATE (SAMPLES/SECOND), DOWN FROM 44100. CALIBRATED WITH 1kHz SIGNAL. 5% OF DATA IS CROPPED OFF, FOR NYQUIST. THESE 4 options ARE FOR CALIBRATION. THIS VERSION ONLY SUPPORTS LR-1kHz. A SINGER'S VOICE CAN GO OVER 1kHz. 
    LR_OVERLAP    =  2,      --DEFAULT=2p (AFTER SCALING DOUBLE). MOVES RIGHT CHANNEL ON TOP OF LEFT, @CENTER=0Hz (DATA DRAG). 
    ...
    freqs_magnification= 1.2, --DEFAULT=1.3  CURVE HEIGHT SCALE FACTOR. REDUCES CPU CONSUMPTION, BUT THE LIPS LOSE TRACTION.    L & R CHANNELS ARE LIKE A DUAL ALIEN MOUTH (LIKE HOW HUMANS ARE BIPEDAL).    TO INCREASE MAGNIFICATION A NEW FILTER IS NEEDED TO STOP EXCESSIVE SHARPNESS.
...
----dynaudnorm   →s64=f:g:p:m:r:n:c:b  DEFAULTS 500:31:.95:10:0:1:0:0    FRAME(MILLISECONDS):GAUSSIAN_WIN_SIZE(ODD INTEGER):PEAK_TARGET[0,1]:MAX_GAIN[1,100]:CORRECTION_DC(BOOL):BOUNDARY_MODE(BOOL,NO FADE)     DYNAMIC AUDIO NORMALIZER. b DISABLES FADE (NOT FOR SPECTRUM). IT MAY SLOW DOWN YOUTUBE, BY PRE-LOADING MANY FRAME-LENGTHS (g=31). LOWER g GIVES FASTER RESPONSE. RENORMALIZING OPTIONAL, AFTER CHANGING samplerate & STARTPTS. ALTERNATIVES INCLUDE loudnorm & acompressor, BUT dynaudnorm IS BEST. 
...
----setpts,asetpts   =expr  PRESENTATION TIMESTAMP.  FOR SYNC OF rotate,zoompan,overlay WITH OTHER GRAPHS (automask), EVEN FOR OFF TIMESTAMPS & YOUTUBE. AFTER SENDING STARTPTS→0, SUBTRACT 1 FRAME FROM TIMESTAMP FRAME [T], BECAUSE THAT TIME IS FOR WHATEVER FOLLOWS IT.   asetpts LEADS THE SPECTRUM BY BACKDATING AUDIO. LAG MAY DEPEND ON averaging size rate ETC.  
...
----anull             RE-LABELER FOR CALIBRATION & TOGGLE OFF. 
...
mp.set_property('script-opts','lavfi-complex=yes,'..mp.get_property('script-opts')) --PREPEND NEW SCRIPT-OPT: WARNING albumart & image WILL INFINITE loop. TRAILING "," ALLOWED.
function file_loaded(event) --ALSO on_aid, on_vid, on_toggle & ytdl.
    ...
    if not vid then complex=                       ('[vid]split[vid],crop=1:1:0:0:1:1,lutyuv=0:128:128:0,scale=%d:%d,setsar=1,format=%s'):format(W,H,o.format) end --CASE 3: RAW MP3. BUILD [vo] FROM BLANK 1x1. scale BEFORE format (BY TRIAL & ERROR).
    if   image then complex=('[vid%d]scale=%d:%d[vo],[vid]split[vid],crop=1:1:0:0:1:1,lutyuv=0:128:128:0[C],[C][vo]scale2ref,overlay,setsar=1,format=%s'):format(vid,W,H,o.format) end --CASE 4: albumart. format,setsar GO AFTER overlay ()!  FOR PROPER MOTION, [vo] GETS SANDWICHED BTWN CANVAS [C] & [vid] (SPECTRUM). 
    
    if o.dual_scale then complex=('%s[vo],[vid]split[vid]%%s,lut=a=val*(%s),scale=%d:%d[v2],[vo][v2]overlay=%s')    -- [v2]=DUAL  LABELS [vid1][vid2] ETC NOT ALLOWED (RESERVED).
