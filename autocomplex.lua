...
mp.set_property('lavfi-complex','anullsrc,anullsink')    --lavfi-complex WARNING FOR OTHER SCRIPTS. CAN'T REFER TO [vid1] NOR [aid1], WHICH MAY NOT EXIST.
...
function file_loaded()
    ...
    if           not vid then complex=('[vid]split[vid],%s'):format(scale) end    --CASE 3: RAW MP3 DOUBLE-SPECTRUM (STILL & MOVING). SIMPLEST CODE IS TO LITERALLY DOUBLE IT.
    if     image and vid then complex=('[vid%d]%s[vo],[vid]split[vid],crop=1:1:1:1:1:1[C],[C][vo]scale2ref,overlay'):format(vid,scale) end    --CASE 4: COVER ART.   FOR PROPER MOTION, [vo] GETS SANDWICHED BTWN [C] & [vid].
