...
options={ --ALL OPTIONAL & MAY BE REMOVED. TO REMOVE A COMPONENT SET ITS alpha TO 0 (freqs, volume, grid & feet).
    ...
    -- bottom_only =true,--REMOVES TOP HALF. TOGGLE THESE 2 LINES FOR NULL OVERRIDE (NO SPECTRAL overlay).
    -- sine_mix={{100,1},{'200:1',2},{300,1},{'400:1',2},{500,1},{'600:1',2},{700,1},{'800:1',2},{900,1},{'1000:1',2}}, --{{frequency(Hz):beep_factor,volume},}  beep_factor OPTIONAL  sine WAVES FOR CALIBRATION-PROOF MIX DIRECTLY INTO [ao].  THIS EXAMPLE BEEPS DOUBLE ON EVEN.   COULD HELP TEST NORMALIZERS, & DECORATE freqs.
    ...
    -- colormix='gb=.4:bb=0', --UNCOMMENT FOR RED/GREEN, INSTEAD OF RED/BLUE (DEFAULT). ADDS ~10% TOTAL CPU USAGE (CAN CHECK TASK MANAGER), EVEN AS A NULL-OP. aa FOR TRANSPARENCY. BLUE ALREADY HAS GREEN gb=.3.    GREY='rr=.7:gr=.7:br=.7:rb=.3:bb=.3'.  BLUE, DARKBLUE & BLACK ARE COMPATIBLE WITH cropdetect (autocrop). RED FOR LIPS. RED & BLUE FOR MAGENTA feet.   A DIFFERENT DESIGN COULD VARIABLY STRIPE SHADES OF BLUE ALONG CURVES (IN GRID FORM?) TO ACHIEVE GOOD CONTRAST WITHOUT EVER USING THE COLOR RED (RESERVED FOR SOMETHING ELSE).
    ...
    highpass  =100, --DEFAULT=100 Hz  DAMPENS SUB-BASS & DC, NEAR volume BAR. 100 Hz FOR BASS HEAVY TRACKS. highpass & dynaudnorm APPLY ONLY TO VISUALS, NOT [ao].  ARGUABLY AMPLITUDES SHOULD BE MULTIPLIED BY FREQUENCY, IN WHICH CASE highpass IS UNNECESSARY.
    dynaudnorm='250:11:1:100:0:1:0:1', --DEFAULT=500=500:31:.95:10:0:1:0:0=f:g:p:m:r:n:c:b  APPLIES TWICE, BEFORE & AFTER aresample & freqs_lead. SEE GRAPH SECTION FOR DETAILS.
    ...
    freqs_lead         = .24, --DEFAULT=.08 SECONDS. LEAD TIME FOR SPECTRUM (TRIAL & ERROR). BACKDATES audio TIMESTAMPS. showfreqs HAS AT LEAST 1 FRAME LAG. DEPENDS ON HUMAN VISION/ACOUSTIC INTERPRETATION TIME, & WHETHER VIEWER IS SQUINTING.
    ...
    freqs_magnification= 1.2, --DEFAULT=1.2  CURVE HEIGHT SCALE FACTOR. REDUCES CPU CONSUMPTION, BUT THE LIPS LOSE TRACTION.    L & R CHANNELS ARE LIKE A DUAL ALIEN MOUTH (LIKE HOW HUMANS ARE BIPEDAL). 
    ...
    shoe_color         ='WHITE@.25', --DEFAULT='BLACK@.25'  BLACK OR WHITE SHOES?   THERE CAN ALSO BE ANOTHER OPTION FOR grid_colormix (BLUE/RED OR RED/BLUE BARS?) RED OUTER BARS ARE BAD FOR cropdetect.
...
for key,val in pairs({toggle_on_double_mute=0,key_bindings='',period=1,fps=25,rotate='0',zoompan='1:0:0',overlay='(W-w)/2:(H-h)/2',dual_alpha=1.5,dual_overlay='(W-w)/2:(H-h)/2',highpass=100,dynaudnorm=500,gb=.3,width=1,freqs_lead=.08,freqs_alpha=.7,freqs_fps=25/2,freqs_mode='line',freqs_win_size=512,freqs_win_func='parzen',freqs_averaging=2,freqs_magnification=1.2,freqs_clip_h=.3,volume_alpha=.25,volume_fade=0,volume_dm=0,volume_width=.04,volume_height=.15,grid_height=.1,grid_thickness=.1,grid_alpha=2,feet_alpha=1,feet_height=.05,feet_activation=.6,shoe_color='BLACK@.25',format='yuv420p',scale={},io_write='',config={}})
...
----highpass         =f:p  Hz:poles  DAMPENS SUB-BASS. ~50Hz IS POWER & REFRESH RATE.   UNFORTUNATELY afftfilt IS TOO BUGGY - IT WOULD ALLOW FINE TUNING SHARP VS. BLAND TONES.
...
----setpts,asetpts   =expr  PRESENTATION TIMESTAMP.  FOR SYNC OF rotate,zoompan,overlay WITH OTHER GRAPHS (automask), EVEN FOR OFF TIMESTAMPS & YOUTUBE. AFTER SENDING STARTPTS→0, SUBTRACT 1 FRAME-TB FROM TIMESTAMP FRAME [t0], BECAUSE THAT TIME IS FOR WHATEVER FOLLOWS IT.   asetpts LEADS THE SPECTRUM BY BACKDATING AUDIO.
...
----select           =expr  (EXPRESSION)  DISCARDS FRAMES IF 0.  OPTIONAL: FOR COMPATIBILITY WITH OLD MPV, select FOR [t0]. REDUCES RAM USAGE.
...
function file_loaded(file_loaded) --ALSO on_aid, on_vid, on_toggle & ytdl.
    ...
    ----5 CASES  1) image & albumart (NO SPECTRUM)  2) MP4 LIMITS (NO SPECTRUM)  3) MP3  4) albumart  5) MP4     INSPECT MPV LOG TO VERIFY EXACT GPU OUTPUT IN EVERY CASE. INSTEAD OF EVERY SCRIPT SETTING ITS OWN lavfi-complex, THEY MAY RELY ON THIS SCRIPT. E.G. automask albumart ANIMATION.
    ...
    if ORIENTATION=='' then return end --set CASES 3,4,5 BELOW. complex VARIABLE MUST PRODUCE %%s=[vo] INSERT.
...
----ISSUE: AMPLITUDES MAYBE SHOULD BE MULTIPLIED BY FREQUENCY. USE afftfilt INSTEAD OF highpass. 400Hz WORTH DOUBLE 200Hz - THAT'S WHY THERE'S A highpass.
