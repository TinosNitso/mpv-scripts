...
function file_loaded()  --ALSO on_aid_vid & on_toggle{ON}.    THIS COULD BE REPLACED BY on_av_params. BUT THE .1s DELAY TO DETECT alpha RARELY CAUSES STUTTER/LAG @LOAD.
    ...
    then OFF       = false  --FORCE TOGGLE OFF, IF OFF.
        ...
        return end  --ON BELOW. 
    if not ov_w then  _,error_ffmpeg =    mp.command('no-osd vf pre    @lavfi-format:lavfi-format')  end  --~ov_w MEANS ONCE ONLY.  OLD FFMPEG DETECTION. MORE RELIABLE THAN VERSION NUMBERS BECAUSE THEY CAN BE ANYTHING. SO DETECT ERROR USING NULL-OP.   BUG: USE lavfi-aformat INSTEAD OF lavfi-format, BECAUSE VIDEO MAY NOT EXIST.
    ...
    format     = (not v.id or v_params.alpha) and (error_ffmpeg and 'yuva420p' or '') or 'yuv420p'  --AKA pixelformat. lavfi-complex CAN'T DETECT WHETHER alpha EVER EXISTED WITHOUT A DELAYED TRIGGER.  overlay FORCES yuva420p, BUT SHOULD REMOVE alpha BECAUSE ITS EXISTENCE MAY TRIGGER BUGS IN VARIOUS SCRIPTS.  OLD FFMPEG REQUIRES IT SPECIFIED.
    vo         = v.id and ('[vid%d]'):format(v.id)..  --3 CASES:  1) PROPER-VIDEO  2) albumart  3) AUDIO-ONLY    
                 (not v.image and ''  --CASE 1: NORMAL VIDEO.  ALMOST ALWAYS [vid1].
    ...
    mp.set_property('lavfi-complex',(lavfi):format(a.id,freqs_rate,freqs_fps,framerate,vo,W,H,ov_w,ov_h,duration,format))  
end 
...
----loudnorm          = I:LRA:TP   DEFAULT -24:7:-2. INTENSITY TARGET (-70 TO -5) : LOUDNESS RANGE (1 TO 20) : TRUE PEAK (-9 TO 0). CAUSED DEFECT REQUIRING apad. LACKS f & g SETTINGS. SOUNDED OFF.  OUTPUTS A BUFFERED STREAM.
----extractplanes     = planes    r+b→[R][L] (RED+BLUE)   REVERSED IF [L] GETS FLIPPED AROUND.
----firequalizer        MAY BE NEEDED TO MULTIPLY BY frequency. (A HIGH PITCHED CHIRP IS DEAFENING TO HUMAN, BUT SAME AMPLITUDE.)
