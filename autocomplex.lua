...
local options={ --ALL OPTIONAL & MAY BE REMOVED. TO REMOVE A COMPONENT SET ITS alpha TO 0 (freqs, volume & feet).
    ...
    volume_height      =  .25, --DEFAULT=.2, RANGE (0,1]. HEIGHT OF BAR (BEFORE STACKING FEET), RELATIVE TO SCREEN.
    ...
    key_bindings         ='F1', --DEFAULT='' (NO TOGGLE). CASE SENSITIVE. FREQS=F BUT F=FULLSCREEN, O=OSD, C=autocrop. V FOR VOLUME? SPECTRUM=S BUT S=SCREENSHOT. 'F1 F2' FOR 2 KEYS.    KEYBOARD TOGGLE WORKS IF MPV HAS ITS OWN WINDOW, BUT NOT BY DEFAULT IN SMPLAYER.
    ...
    config={
            'keepaspect no','geometry 50%',   --ONLY NEEDED IF MPV HAS ITS OWN WINDOW, OUTSIDE SMPLAYER. ANY aspect & 50% INITIAL SIZE.
            'vd-lavc-threads 0','hwdec auto-copy','video-timing-offset 1',   --IMPROVED PERFORMANCE IN LINUX. vd-lavc=VIDEO DECODER-LIBRARY AUDIO VIDEO CORES (0=AUTO).  hwdec=HARDWARE DECODER.  DEFAULT offset=.05 SECONDS (RENDERING TIME). 
...
mp.set_property('lavfi-complex','anullsrc,anullsink')    --NULL lavfi-complex ESTABLISHES IT, SO OTHER SCRIPTS DON'T. 
...
----lavfi            =graph  (SPECTRUM) LIBRARY-AUDIO-VIDEO-FILTER LIST. SELECT FILTER NAME TO HIGHLIGHT IT (NO WORD-WRAP). SOME APPEAR AS PAIRS/TRIPLES. lavfi-complex MAY COMBINE MANY audio & video INPUTS. %%d SUBS OCCUR LATER. A lavfi string IS LIKE DNA & CAN CREATE ANY CREATURE. SEE ffmpeg-filters MANUAL.   ALL THE TIMESTAMP FRAME [T] CODE EXISTS JUST TO SYNC THE MOVING SPECTRUM WITH OTHER GRAPHS, LIKE automask. 
----split,asplit     =outputs  (DEFAULT 2)  CLONES audio,video. aid MAY BE KNOWN IN ADVANCE, BUT NOT NECESSARILY.
----format,aformat   =pix_fmts,sample_fmts:sample_rates:channel_layouts      format CONVERTS BTWN rgb24 rgb32. yuv444p IS (PROBABLY) MORE OPTIMAL, BUT TOO HARD TO USE.  aformat CONVERTS TO STEREO, FROM MONO & SURROUND (FOR VISUALS). DOWNSAMPLES TO ~2.1kHz (NYQUIST+.5%). u8 CAUSES HISSING COMPARED TO s16, WHICH MAY BE MORE EFFICIENT THAN s64.
...
----dynaudnorm       =...:p:m:...:c:b  DEFAULTS .95:10:0:0    PEAK TARGET [0,1] : MAX GAIN [1,100] : CORRECTION (DC,0Hz) BOOL : BOUNDARY MODE (NO FADE) BOOL    NORMALIZES & RENORMALIZES. b DISABLES FADE, WHICH IS BAD FOR SPECTRUM. OUTPUTS s64.   ALTERNATIVES INCLUDE loudnorm & acompressor.
...
function file_loaded()
    ...
    CANVAS=('asplit[ao],aformat=s16:128:mono,showwaves=1x1:r=%s:colors=BLACK@0,scale=%d:%d'):format(o.fps,W,H)    --CASES 3 & 4 (& TOGGLE). SPLITS audio STREAM TO BLANK CANVAS. 
