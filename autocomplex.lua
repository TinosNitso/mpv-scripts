...
options={ --ALL OPTIONAL & MAY BE REMOVED. TO REMOVE A COMPONENT SET ITS alpha TO 0 (freqs, volume, grid & feet).
    ...
    period=19/25, --DEFAULT=1 SECOND. SET TO 0 FOR STATIONARY. USE fps RATIO (E.G. 19/25=79BPM, BEATS PER MINUTE). UNLIKE A MASK, MOTION MAY NOT BE PERIODIC - ENSEMBLE MAY FREELY FLOAT AROUND. (IF 0, "n/%s"→"0" GSUBS OCCUR, ETC). 
    fps   =   25, --DEFAULT=25 FRAMES PER SECOND. SCRIPT LIMITS fps & scale. 
    
    -- colormix='gb=.3:bb=0',     --UNCOMMENT FOR RED/GREEN, INSTEAD OF RED/BLUE (DEFAULT). ADDS 10% CPU USAGE (CAN CHECK TASK MANAGER). aa FOR TRANSPARENCY. BLUE ALREADY HAS GREEN gb=30%, +30% → NEUTRAL GREEN.    GREY='rr=.7:gr=.7:br=.7:rb=.3:gb=.3:bb=.3'.  BLUE, DARKBLUE & BLACK HIGHS ARE COMPATIBLE WITH cropdetect (autocrop). RED FOR LIPS. RED & BLUE MIX IN THE feet. 
    ...
    overlay='(W-w)/2:H*(.75+.05*(1-cos(2*PI*n/%s))*mod(floor(n/%s)+1\\,2))-h/2',--DEFAULT=(W-w)/2:(H-h)/2  %s=fps*period  mod ACTS AS ON/OFF SWITCH.   SPECTRUM y FROM SCREEN TOP (RATIO), BEFORE autocrop.     THIS EXAMPLE MOVES DOWN→UP→RIGHT→LEFT BY MODDING. TIME-DEPENDENCE SHOULD MATCH OTHER SCRIPT/S, LIKE automask. A BIG-SCREEN TV HELPS WITH POSITIONING.  THE IDEAL COMPLEX COULD BE POSITIONED BY GAMEPAD CONTROLLER, BUT NOT THIS ONE.
    
    -- dual_colormix='gb=.3:bb=0',--UNCOMMENT FOR RED/GREEN DUAL. ADDS 5% CPU USAGE.  MAGENTA/GREEN='gb=.3:bb=0:br=.8:rr=.8'
    dual_alpha   =1.42,             --DEFAULT=1.5  alpha MULTIPLIER. MORE EFFICIENT THAN dual_colormix.
    dual_scale   ={.8,.8},          --RATIOS {WIDTH,HEIGHT}. REMOVE FOR NO DUAL. FULL BI-QUAD CONCEPT COMES FROM HOW RAW MP3 WORKS.   IT ALSO EMULATES THE "THIRD EYE", WHICH MIRRORS THE MOUTH.   IT'S ALSO POSSIBLE TO ADD A 3RD LITTLE COMPLEX ON TOP, LIKE PYRAMID.
    ...
    gb      = .3, --DEFAULT= .3  RANGE [-2,2]  PERCENTAGE OF GREEN IN BLUE, BEFORE colormix. PROPERLY COMPLEMENTING RED REQUIRES SOME GREEN. BLUE+0%GREEN IS TOO DARK. MOST EFFICIENT CODE SHOULD START WITH CORRECT BLUE/RED SHADES, WITHOUT EXTRA colormix.
    ...
    freqs_fps          =25/2, --DEFAULT=25/2  25fps CAUSES LAG. THIS & freqs_clip_h HELP WITH PERFORMANCE.
    ...
    feet_threshold_h= .6, --DEFAULT= .6  RANGE [0,1), RELATIVE TO volume. feet BLINK ON/OFF WHEN volume PASSES THIS THRESHOLD (FROM MAX). THEY'RE CROPPED @THIS HEIGHT, FROM TOP. ALL feet CUT THE SAME.
...
lavfi=('[aid%%d]%sasplit[ao],asettb=1/(%s),stereotools,highpass=%s,dynaudnorm=500:5:1:100:0:0:1:1,asplit[af],showvolume=%s:0:128:8:%s:t=0:v=0:o=v:dm=%s,colorchannelmixer=aa=%s:bg=1:gg=%s,split=3[vol][BAR],crop=iw/2*3/4:ih*(%s):(iw-ow)/2:ih*(%s),lut=a=val*(%s):g=0,pad=iw*4/3:ih+(ow-iw)/a:(ow-iw)/2:oh-ih:%s,split,hstack,split[feet0],colorchannelmixer=rb=1:br=1:rr=0:bb=0:gg=0:gr=%s[feet],[vol][feet0]vstack[vol],[BAR][feet]vstack,lut=a=val*(%s),split[RGRID],crop=iw/2:ih:0,pad=iw/%s:0:0:0:BLACK@0,split=10,hstack=10,crop=iw-4:ih:iw-ow,pad=iw+4:0:0:0:BLACK@0[LGRID],[RGRID]crop=iw/2:ih:iw/2,pad=iw/%s:0:ow-iw:0:BLACK@0,split=10,hstack=10,crop=iw-4:ih:0,pad=iw+4:0:ow-iw:0:BLACK@0[RGRID],[LGRID][RGRID]hstack,pad=0:ih/(%s):0:oh-ih:BLACK@0[grid],[af]aresample=%s,asetpts=PTS-(%s)/TB,apad,showfreqs=300x512:mode=%s:ascale=lin:fscale=lin:win_size=%s:win_func=%s:averaging=%s:colors=BLUE|RED,fps=%s,crop=iw/1.05:ih*(%s):0:ih-oh,format=rgb24,scale=iw*2:-1,avgblur,lut=255*gt(val\\,140):0:255*gt(val\\,140),avgblur=2,lut=255*gt(val\\,90):0:255*gt(val\\,90),scale=ceil(iw/4)*4:ceil(ih/4)*4,format=rgb32,split[L],colorchannelmixer=bb=0:aa=0:ar=%s[R],[L]colorchannelmixer=bb=0:aa=0:ab=%s:rb=1:rr=0,hflip[L],[grid][L]scale2ref=iw*2-(%s):ih,overlay=0:0:endall[grid+L],[grid+L][R]overlay=W-w:0:endall,scale=ceil(iw/4)*4:ih,split=3[RHIGH][LOWS],crop=iw/4:ih:0,colorchannelmixer=rb=1:br=1:rr=0:bb=0:gg=0:gr=%s[LHIGH],[LOWS]crop=iw/2[LOWS],[RHIGH]crop=iw/4:ih:iw-ow,colorchannelmixer=rb=1:br=1:rr=0:bb=0:gg=0:gr=%s[RHIGH],[LHIGH][LOWS][RHIGH]hstack=3[vid],[vol][vid]scale2ref=round(iw*(%s)/4)*4:round(ih*(%s)/4)*4[vol][vid],[vid][vol]overlay=(W-w)/2:H-h,%s[vid],%%s,split=3[C][T],setpts=PTS-STARTPTS[vo],[C]crop=1:1:0:0:1:1,format=ya16,lut=a=0[C],[C][vid]scale2ref,overlay,setpts=PTS-STARTPTS,rotate=%s:iw:ih:BLACK@0,zoompan=%s:0:%%dx%%d:%s[vid],[vo][vid]overlay=%s[vo],[T]select=lt(n\\,2),trim=end_frame=1,setpts=PTS-1/FRAME_RATE/TB[T],[T][vo]concat,trim=start_frame=1,format=%s[vo]')
    :format(CALIBRATION,o.fps,o.highpass,o.fps,o.volume_fade,o.volume_dm,o.volume_alpha,o.gb,o.feet_height,o.feet_threshold_h,o.feet_alpha,o.SHOE_COLOR,o.gb,o.grid_alpha,o.grid_thickness,o.grid_thickness,f(o.grid_height/o.freqs_clip_h),o.freqs_resample,o.freqs_lead,o.freqs_mode,o.freqs_win_size,o.freqs_win_func,o.freqs_averaging,o.freqs_fps,f(o.freqs_clip_h/o.freqs_magnification),o.freqs_alpha,o.freqs_alpha,o.LR_OVERLAP,o.gb,o.gb,o.volume_width,f(o.volume_height/o.freqs_clip_h),ORIENTATION,o.rotate,o.zoompan,o.fps,o.overlay,o.format)    --fps FOR volume, freqs & zoompan. feet_alpha REPEATS FOR INNER & OUTER [FEET]. freqs_clip_h CROPS freqs & PADS volume & [grid]. freqs_alpha REPEATS FOR L & R CHANNELS. 

----lavfi            =graph  LIBRARY-AUDIO-VIDEO-FILTER LIST. SELECT FILTER NAME TO HIGHLIGHT IT. NO WORD-WRAP → SIDE-SCROLL PROGRAMMING, WITH LINE DUPLICATION ETC. lavfi-complex MAY COMBINE MANY [aid#] & [vid#] INPUTS. %% SUBS OCCUR LATER. A lavfi string IS LIKE DNA & CAN CREATE ANY CREATURE. SEE ffmpeg-filters MANUAL.  EACH FOOT HAS A STEREO IN IT, IN THIS VERSION. [feet] ARE THE CENTER-PIECE OF THE WHOLE THING.  [T] CODE ALWAYS VALID, EVEN ON YOUTUBE & FOR MP4 SUBCLIPS WITH OFF STARTPTS. THIS UPDATE USES CANVAS [C] FOR zoompan, SO THAT IT'S ALWAYS EXACTLY IN SYNC. [C] SWITCHES OUT audio TIMESTAMPS WITH [vo].
...
----dynaudnorm   →s64=f:g:p:m:r:n:c:b  DEFAULTS 500:31:.95:10:0:1:0:0    GAUSSIAN_WIN_SIZE(ODD INTEGER):PEAK_TARGET[0,1]:MAX_GAIN[1,100]:CORRECTION_DC(BOOL):BOUNDARY_MODE(BOOL,NO FADE)     DYNAMIC AUDIO NORMALIZER. b DISABLES FADE (NOT FOR SPECTRUM). IT MAY SLOW DOWN YOUTUBE, BY PRE-LOADING MANY FRAME-LENGTHS (g=31). LOWER g GIVES FASTER RESPONSE. f CAUSES BUG (BAD PERFORMANCE). ALTERNATIVES INCLUDE loudnorm & acompressor, BUT THEY ARE WORSE. 
...
----showfreqs [a]→[v]=size:RATE:mode:ascale:fscale:win_size:win_func:overlap:averaging:colors  DEFAULTS 1024x512:25:bar:log:lin:2048:hanning:1:1   RATE CRASHES LINUX snap, SO THESE NEED TO BE SPELLED OUT FOR COMPATIBILITY. ASPECT SHOULD BE APPROX .5 FOR SHARPEST CURVE TO BE EQUALLY THICK IN HORIZONTAL & VERTICAL (E.G. 256x512 NOT 256x256 NOR 256x1024). 256x512 SCALED UP LOOKS BETTER THAN 512x1024 (TRIAL & ERROR). SEPARATING CHANNELS WITHOUT COLORS (cmode=separate) WOULD REQUIRE TWICE AS MANY PIXELS.
...
----crop             =out_w:out_h:x:y:keep_aspect:exact    CENTERS BY DEFAULT. ZOOMS IN ON ascale & fscale. REMOVES MIDDLE TICK ON GRID. SEPARATES [LOWS].
----lut              =...:a  LOOK-UP-TABLE  lutrgb MAYBE SLOWER THAN lutyuv (CHECK STEADY-CAM FOOTAGE). CREATE TRANSPARENCY, & SELECTS CURVE FROM BLUR BRIGHTNESS. CURVE SMOOTHNESS & THICKNESS DOUBLE-CALIBRATED USING lutrgb>140 & 90.  WITH THINNER CURVES MORE CUMULATIVE VISUALS CAN BE ADDED.
...
----setsar           =sar  STOPS EMBEDDED MPV SNAPPING (CAN VERIFY WITH DOUBLE-mute TOGGLE IN SMPLAYER). MACOS BUGFIX REQUIRES sar.
----format           =pix_fmts  CONVERTS BTWN rgb32 rgb24 (MORE EFFICIENT). 
...
function file_loaded(event)  --ALSO on_aid, on_vid, on_toggle & ytdl.
    if mp.get_property_number('time-remaining')==0 then mp.command('stop') end  --BUGFIX FOR seek PASSED end-file. A CONVENIENT WAY TO SKIP NEXT TRACK IN SMPLAYER IS TO SKIP 10 MINUTES PASSED end-file. THIS LINE IS SIMPLE SOLUTION.
    time,aid,vid = mp.get_property_number('time-pos'),mp.get_property_number('current-tracks/audio/id'),mp.get_property_number('current-tracks/video/id') --aid & vid MAYBE= 'auto', BUT id=nil OR INTEGER. 
    if OFF or last_time==time then return end --DON'T RELOAD IF OFF OR DOUBLE-TRIGGERED.
    last_time,image,complex = time,false,nil  --last_time=time@LOAD, image=BOOL, complex=GRAPH SUBSTRING
    ...
    clip_h,scale = math.ceil(H*o.freqs_clip_h*2/4)*4,('scale=%d:%d,setsar=1,format=%s'):format(W,H,o.format)   --FINAL CLIP HEIGHT FOR TOP & PADDED BOTTOM (*2). scale FOR EVERY CASE.
    if not aid then ORIENTATION='' end
    
    ----5 CASES. 1) JPEG & COVER ART (NO SPECTRUM)  2) MP4 LIMITS (NO SPECTRUM)  3) MP3  4) MP3+JPEG  5) MP4     INSTEAD OF EVERY SCRIPT SETTING ITS OWN lavfi-complex, THEY MAY RELY ON THIS SCRIPT. E.G. automask JPEG ANIME USES MORE CPU IF COMBINED WITH autocomplex.
    if             image and ORIENTATION=='' then complex=('[vid%d]%s,loop=-1:1,fps=%s:%s[vo]'):format(vid,scale,o.fps,time) end   --CASE 1: RAW JPEG. start_time FOR --start (JPEG seek).
...
mp.register_event('seek'       ,file_loaded)   --BUGFIX: RESET GRAPH ON seek.

function on_aid(_,aid)  --ONLY GOOD FOR 1 CHANGE IN aid OR vid, BTWN 1 & 2.
    if last_aid and last_aid~=aid then new_aid=1
        if          last_aid==1   then new_aid=2 end
        file_loaded() end
    last_aid=aid    --REMEMBER aid.
end
mp.observe_property('aid','number',on_aid)  --TRIGGERS INSTANTLY & AFTER file-loaded.

function on_vid(_,vid)  --ONLY GOOD FOR 1 CHANGE.
...
function on_toggle(mute)    --WON'T CHANGE vid OR aid WHEN TOGGLED OFF (BUG).
    ...
    OFF,last_time = not OFF,nil --REMEMBER TOGGLE STATE, FORGET time.
    if not OFF then file_loaded()   --TOGGLE ON.    UNFORTUNATELY THIS SNAPS COVER ART IN SMPLAYER (EVERY TIME).
    elseif ORIENTATION~='' then --CASES 3,4 & 5 TOGGLE OFF BELOW. CASES 1 & 2 DO NOTHING. OPEN TO INTERPRETATION: NO SPECTRUM, NO TOGGLE OFF.   1) JPEG. 2) MP4 LIMITS. 3) MP3 SPECTRUM. 4) MP3+JPEG. 5) MP4.
