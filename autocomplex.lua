----lavfi-complex SCRIPT WHICH LIMITS fps & scale; LOOPS albumart; & OVERLAYS STEREO FREQUENCY SPECTRUM + volume BARS (AUDIO VISUALS) ONTO MP4, AVI, MP3 (RAW & albumart), MP2, M4A, WAV, OGG, AC3, OPUS & YOUTUBE.  
...
o={ --options  ALL OPTIONAL & MAY BE REMOVED.   TO REMOVE AN INTERNAL COMPONENT SET ITS alpha TO 0 (freqs volume grid feet shoe).
    ...
    -- period=0, --DEFAULT= 1 SECOND. SET TO 0 FOR STATIONARY. USE fps RATIO. 22/30→82BPM, BEATS PER MINUTE. UNLIKE A MASK, MOTION MAY NOT BE PERIODIC - ENSEMBLE FREE TO RANDOMLY FLOAT AROUND. (IF 0, "n/%s"→"0" GSUBS OCCUR, ETC). 
    ...
    freqs_fps          =25/2, --DEFAULT=25/2 ONLY FOR STANDARD MP4. image & RAW MP3 OPERATE @volume_fps (DOUBLE). 25fps CAUSES LAG. freqs_clip_h ALSO IMPROVES PERFORMANCE.  SLOW-MO WORKS WELL ENOUGH WITH SLOW AUDIO.   
    ...
    -- freqs_interpolation=true,--UNCOMMENT FOR INTERPOLATION (freqs_fade). ADDS ~7% CPU USAGE. NICE EFFECT BUT UNFORTUNATELY MAY CAUSE STUTTER @autocrop (BY TRIAL & ERROR).
    freqs_alpha        =  .7, --DEFAULT= .7  RANGE [-2,2]  OPAQUENESS OF SPECTRAL DATA CURVE.    DUAL-complex MAY LOOK BETTER TRANSPARENT.
    volume_fps         =  25, --DEFAULT= 25  PRIMARY overlay fps. OPTIMAL @25fps, FILM OPTIMAL @30fps! SO REPEAT EVERY FIFTH VISUAL FRAME.
    ...
    feet_lutrgb        ='128:0:255:val*4', --DEFAULT=128:0:255:val*2  *0 REMOVES feet. COLOR OF CENTRAL feet.
    ...
    options=''           --'opt1 val1 opt2 val2 '... FREE FORM.  main.lua HAS io_write & options.
        ..' vd-lavc-threads 0  ' --vd-lavc=VIDEO DECODER - LIBRARY AUDIO VIDEO. 0=AUTO OVERRIDES SMPLAYER, OR ELSE MAY FAIL INSPECTION.
}
...
mp.set_property('script-opts','lavfi-complex=yes,'..mp.get_property('script-opts')) --WARN ALL SCRIPTS image WILL INFINITE loop INSIDE complex. A complex loop IS A MORE POWERFUL loop THAN OTHERWISE POSSIBLE.

if o.period==0 or o.period=='0' then for opt in ('rotate zoompan overlay dual_overlay'):gmatch('%g+')  --DON'T DIVIDE BY 0 BY REMOVING TIME DEPENDENCE.
    do for nt in ('in on n t')     :gmatch('%g+') do o[opt]=o[opt]:gsub(nt..'/%%s','0') end end end    --in & on BEFORE n.  OVERRIDE: THIS CODE ONLY GSUBS "t/%s" ETC (NOT FULLY GENERAL).
opt=nil
for opt in ('rotate  zoompan     '):gmatch('%g+') do o[opt]=o[opt]:gsub('%%s',('(%s*%s)'):format(o.period,o.volume_fps)) end    --OPTIMIZE USING DIFFERENT fps FOR ANIMATION VS FINAL FILM overlay.
for opt in ('overlay dual_overlay'):gmatch('%g+') do o[opt]=o[opt]:gsub('%%s',('(%s*%s)'):format(o.period,o.       fps)) end
...
----shuffleplanes    =map0:map1:map2:map3  REDUCES NET CPU USAGE BY ~5% (ESSENTIAL). ORDERED g:b:r:a (LIKE GreatBRitAin). SHUFFLES WITHOUT MIXING. DEFAULT 0:1:2:3  LINUX snap COMPATIBILITY DEPENDS ON EXACT USAGE.  SWITCHES HIGHS FROM LOWS, & SEPARATES L & R CHANNELS.
...
function file_loaded(loaded) --ALSO on_aid, on_vid, on_toggle & ytdl.
    image,aid,vid,script_opts = false,mp.get_property_number('current-tracks/audio/id'),mp.get_property_number('current-tracks/video/id'),mp.get_property_native('script-opts') --image=BOOL  'aid' & 'vid' MAYBE='auto', BUT id=nil OR INTEGER. 
    if loaded then new_aid,new_vid = nil,nil end  --RESET NEW ID SWITCHES.
    ...
    for _,track in pairs(mp.get_property_native('track-list')) do if track.type=='video' and track.id==vid then image,w,h,par = track.image,track['demux-w'],track['demux-h'],track['demux-par'] --LOOP OVER ALL TRACKS TO CHECK IF vid IS image.
            if not par then par=1 end   --JPEG: ASSUME 1=PIXEL ASPECT RATIO.
            if w and h then script_opts.aspect=w/h..''  --REPORT TRUE aspect TO autocrop (vid BUG). CHECK demux-w & h BECAUSE THEY MAY FAIL TO UPDATE WHEN SWITCHING vid, BTWN VIDEO & LEAD-FRAME.
                mp.set_property_native('script-opts',script_opts) end  --REPORT ORIGINAL aspect TO ALL OTHER SCRIPTS.
            break end end 
    ...
    -- mp.command('set osd-font-size 16')
    -- mp.osd_message(lavfi,60)
    lavfi=lavfi:format(aid,freqs_fps,framerate,complex,W*o.width,clip_h,'','','','','','','')
    mp.set_property('lavfi-complex',lavfi)  --CASES 3,4,5.  size FOR zoompan.  %s='' TERMINATES FORMATTING.
end 
...
mp.register_event('end-file',function() last_aid,last_vid = nil,nil end)  --FOR MPV PLAYLIST.

function on_aid(_,aid)  --UNTESTED. ONLY GOOD FOR 1 SWITCH.  INCOMPATIBLE WITH aspeed.lua (CURRENT-TRACK DOESN'T CHANGE OVER).
...
function on_toggle(mute)
    ...
    elseif vstack~='' then complex=('[aid%d]anull[ao]'):format(aid) --TOGGLE OFF CASE 3: RAW audio. STILL FRAME SPECTRUM, audio PLAYS ON.  CASES 4 & 5 BUILD ON THIS.   CASES 1 & 2 DO NOTHING. OPEN TO INTERPRETATION: NO SPECTRUM, NO TOGGLE OFF.   1) JPEG. 2) MP4 LIMITS. 3) MP3 SPECTRUM. 4) MP3+JPEG. 5) MP4.
