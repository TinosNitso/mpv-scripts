...
options                    = {
    ...
    TOLERANCE              = {.05,time=10},  --{0} DEACTIVATES THIS.  INSTANTANEOUS TOLERANCE. WHAT PERCENTAGE BLACK BARS ARE TOLERATED FOR UP TO time=10 SECONDS? A BIG crop IS INSTANT, BUT NOT LITTLE CROPS, OTHERWISE AN IMAGE MAY KEEP FIDGETING BY A PIXEL OR TWO.
    ...
    -- toggle_expr         = '(1-cos(PI*%s))/2',  --UNCOMMENT FOR NON-LINEAR SINUSOIDAL TRANSITION BTWN aspect RATIOS (HALF-WAVE).  A SINE WAVE IS 57% FASTER @MAX-SPEED (PI/2=1.57), FOR SAME DURATION. BUT ACCELERATION MAY LOOK BAD.
    ...
    video_out_params       =  {w,h,pixelformat},  --OVERRIDES.  USE {pixelformat='yuva420p'} FOR TRANSPARENT PADDING.  DEFAULT w,h = display-width,display-height  OR  width,height.  EMBEDDED MPV MAY HAVE display-width=nil.  EXAMPLE: {w=1680,h=1050}
    options                =  {
...
require 'mp.options'.read_options(o) --mp    = MEDIA-PLAYER  ALL options SHOULD BE WELL-DEFINED. COMPULSORY IS SIMPLER. HENCE SOME ARE BLANKS.
...
aspects               = {}  --{ON,OFF,restart,osd,video} FOR PADDING.  ON, OFF & restart ARE RELATIVE VALUES.
...
function file_loaded()  --ALSO @property_handler.
    ...
    W                     = o.video_out_params.w or p['display-width' ] or p.width  or v['demux-w'] --OVERRIDE  OR  display  OR  VIDEO-DIMENSIONS.  width SOMETIMES BUGS OUT @file-loaded.
    ...
    ):format(command_prefix,label,W,H,command_prefix,label,command_prefix,label,detect_format,detector,command_prefix,p['time-pos']))
    ...
    ----pad        = w:h:x:y:color             DEFAULT=0:0:0:0:BLACK  0 MAY MEAN iw OR ih.  @reload_pad BELOW.  FOR TOGGLE OFF. RETURNS EXTRA BLACK BARS TO CORRECT aspect. MPV HAS A WINDOW TO COLOR IN.
    ----setsar     = sar                       DEFAULT=0  IS THE FINISH.  SAMPLE ASPECT RATIO=1 FINALIZES OUTPUT DIMENSIONS. OTHERWISE THE DIMENSIONS ARE ABSTRACT RESOLUTION (PIXELS CAN HAVE ANY SHAPE).  ALSO PREVENTS EMBEDDED MPV FROM SNAPPING ON albumart.  FOR DEBUGGING CAN PLACE setsar=1 EVERYWHERE (ACTS AS GRAPH-ARMOR).  THIS IS LIKE 2 SCRIPTS COMBINED, & setsar=1 IS REPEATED THE SAME WAY, FOR RELIABILITY. ALL OTHER SCRIPTS WORK FINE WITHOUT setsar=1!
...
function reload_pad()  --@property_handler & @seek.  DELAYED TRIGGER BECAUSE MPV REQUIRES EXTRA ~.1s TO DETECT osd-dimensions/aspect>0, & PADDING APPENDS AFTER OTHER SCRIPT/S.  
    ...
    mp.command((''
                ...
                ..              '%s vf  append @%s-pad:lavfi=[format=%s,pad=%d:%d:(ow-iw)/2:(oh-ih)/2:%s,scale=%d:%d,setsar=1];' --FINAL scale ALMOST ALWAYS NULL-OP, UNLESS W OR H ODD (setsar=1). 
                ..(not p.pause and 'set pause  no ;' or '')
    ):format(command_prefix,label,pad_iw,pad_ih,o.pad_scale_flags,command_prefix,label,format,W2,H2,o.pad_color,W,H))
end 
...
function apply_pad(aspect)  --@playback-restart, @property_handler, @on_toggle_pad & @on_toggle.  SIMPLER TO SEPARATE UTILITY FROM ITS TOGGLE.  STRETCHES BLACK BARS WITHOUT CHANGING [vo] scale. 
    ...
    if not (W2 and aspects.video and p['time-pos']) then return end  --time-pos=nil WHEN PASSED end-file, BUT playback-restart TRIGGERS.  aspects.video=nil DURING complex TRACK-CHANGES.  W2=nil IF JPEG STARTS BEFORE osd OBSERVATION.
...
function apply_crop(meta)  --@detect_crop & @on_toggle.
    ...
    meta.min_w    =  meta.min_w        or round(meta.max_w*detect_min_ratio) --RECOMPUTE WHEN NEEDED.
    meta.min_h    =  meta.min_h        or round(meta.max_h*detect_min_ratio)
...
function detect_crop()  --@timers.auto_delay, @playback-restart & @on_toggle.
    ...
    show_text = o.meta_osd and mp.command(('show-text                    "'
                    ...
                    ..'    min_w,min_h      = %s,%s                   \n\n'  --480,270
                    ..'vf-metadata/%s = \n${vf-metadata/%s}              "'            
                ):format(_VERSION,pad_iw,pad_ih,W2,H2,meta.min_w or '',meta.min_h or '',label,label))  --nil INVALID ON MPV.APP.  min_* RESET @property_handler.
