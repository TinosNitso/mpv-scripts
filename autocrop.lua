...
options                    = { 
    ...
    double_mute_timeout    =        .5   , --SECONDS FOR DOUBLE-MUTE TOGGLE (m&m DOUBLE-TAP).  0 MEANS INACTIVE.  TRIPLE-MUTE DOUBLES BACK.  SCRIPTS CAN BE SIMULTANEOUSLY TOGGLED USING DOUBLE-MUTE.  REQUIRES AUDIO IN SMPLAYER.  SCREEN DOUBLE-TAP MAY BE SMOOTHER FOR ANDROID (AVOID DOUBLE-MUTE).
...
function playback_restart()              --FILTERS' STATES ARE RESET, UNLESS is1frame. 
    ...
    m.w,m.h,m.x,m.y,m.aspect,m.time_pos = p.width,p.height,0,0,nil  --restart MEMORY STATE.  ~time_pos FORCES vf-COMMANDS.
...
function on_seek()  --DETECTS BACKWARDS-seek, & MAY REPLACE PAD & LOOP, UNLESS is1frame.
    if is1frame then return end
...
function property_handler(property,val)
    ...
    double_mute =       (property=='mute' or property=='current-tracks/audio/selected') and W2 and (not timers.mute:is_enabled() and (timers.mute:resume() or 1) or on_toggle())  --ANDROID MUTES BY TRACK DE-SELECTION.  W2 STOPS FLIPPING ON RAW AUDIO.  ADDING A playback_restarted CONDITION WOULD BE MORE RIGOROUS.  SMPLAYER DOUBLE-MUTE WHILE seeking MAY FAIL (CANCELS ITSELF OUT).  THERE MAY BE A BETTER TECHNIQUE FOR SMARTPHONES (DOUBLE-MUTE TAKES 3 TAPS!).
...
function detect_crop()  --@timers.auto_delay, @playback-restart & @on_toggle.
    ...
    if not (W2 and p.width and p.height) then return end --width=nil IF TOGGLING vo.  W2 FOR show-text.
...
----A FUTURE VERSION SHOULD BE CAPABLE OF SMOOTH-CROPPING BY REPLACING BLACK BARS WITH BLACK PADS, & THEN SMOOTH-UNPAD. BECAUSE SMOOTH-PADDING IS EASY.  EFFICIENT TRUE SMOOTH-crop IS IMPOSSIBLE WITH FFMPEG-v6.
----o.keep_aspect  SHOULD BE IN A FUTURE VERSION, TO MAINTAIN TRUE ASPECT @ALL TIMES.
