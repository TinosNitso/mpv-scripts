----SMOOTHLY AUTO-CROP BLACKBARS OFF JPEG, PNG, BMP, GIF, MP3 (COVER ART), & MP4. .TIFF ONE-LAYER ONLY. CAN CHANGE vid TRACK (MP3TAG) & crop. FOR RAW JPEG SMOOTHCROP NEED autocomplex LOOP.
----USE DOUBLE-mute TO TOGGLE. CAN MAINTAIN CENTER IN HORIZONTAL/VERTICAL, WITH TOLERANCE VALUES. ALSO SUPPORTS MULTIPLE DETECTORS, & bbox. 
...
options = {
    ...
    key_bindings         ='C c J j',--DEFAULT='C'. CASE SENSITIVE. C→J IN DVORAK, BUT J IS ALSO next_subtitle (& IN VLC IS dec_audio_delay). M IS MUTE SO CAN DOUBLE-PRESS M. M&M OUGHT TO BE automask, BUT autocrop IS MORE FUNDAMENTAL. key_bindings DON'T WORK INSIDE SMPLAYER. 
...
o,label,timers = options,mp.get_script_name(),{} --ABBREV. options. label=autocrop   
m,unpause = {},not mp.get_property_bool('pause') --m=crop MEMORY  ALSO pause STATE.
...
function file_loaded()
    ...
    mp.command(('%s vf pre @%s-crop:lavfi=[scale=max(%d\\,iw):max(%d\\,ih),crop=%d:%d:0:0:1:1,setsar=1]'):format(o.command_prefix,label,W,H,W,H))
    mp.command(('%s vf pre @%s:lavfi=[noformat=rgb32|yuva420p|yuva444p,%s,split,overlay,scale=%d:%d:eval=frame]'):format(o.command_prefix,label,o.detectors,W,H))   --CENTRAL GRAPH. USES MOST CPU. BY TRIAL & ERROR NEEDS width & height INSTANTLY TO AVOID RARE STARTING GLITCH.
    ...
    ----noformat  =pix_fmts  OPTIONAL: BLOCKS alpha. cropdetect MAY PERFORM BADLY WITH TRANSPARENT INPUT (EVEN IF 100% OPAQUE). THERE'S A LONG LIST.
...
function apply_crop(meta) 
    ...
    if is1frame then timers.auto_delay:kill()   --audio GLITCHES ON 1 FRAME crop, SO kill timer.
        ...
        mp.command(('%s vf pre @%s:lavfi=[%s,crop=%d:%d:%d:%d:1:1,scale=%d:%d]')   --REPLACEMENT GRAPH. TOGGLE "C" REPEATEDLY (WITH &) WITHOUT autocomplex, TO VERIFY.  REMOVING ANY GRAPH COULD GET THEM IN THE WRONG ORDER ON vid CHANGE OR TOGGLE (2 GRAPHS + FILTER + automask CAN BE DIFFICULT TO ORDER). 
            :format(o.command_prefix,label,o.detectors,meta.out_w,meta.out_h,meta.x,meta.y,W,H))
        mp.command('set pause no') --image-display-duration=inf
        return end --video BELOW.
    
    s=('clip((t-%s)/(%s),0,1)'):format(mp.get_property_number('time-pos'),o.crop_time)  --ABBREV. TIME RATIO AS s=string.  clip MOST RELIABLE.
    ...
    mp.command(('vf-command %s width  iw/((1-%s)*(%s)+%s*(%s))*%d'):format(label,s,m.out_w,s,meta.out_w,W))
    mp.command(('vf-command %s height ih/((1-%s)*(%s)+%s*(%s))*%d'):format(label,s,m.out_h,s,meta.out_h,H))
    m.x,m.y,m.out_w,m.out_h = meta.x,meta.y,meta.out_w,meta.out_h   --meta→m  MEMORY TRANSFER. is1frame DOESN'T NEED IT.
    
    if mp.get_property_bool('pause') then N=0 --vf-command BUGFIX FOR PAUSED MP4, SIMILAR TO automask. CAN VERIFY VALID FOR RAW JPEG TOO. frame-step IS FASTER & SIMPLER THAN FULL GRAPH TOGGLE.
        while N<5 do N=N+1                    
