...
----THIS VERSION HAS SMOOTH AUTO-aspect (EXTRA BLACK OR COLORED DYNAMIC BARS), BUT NOT SMOOTH-CROP.  BASED ON https://github.com/mpv-player/mpv/blob/master/TOOLS/lua/autocrop.lua  BUT WITHOUT video-crop BECAUSE THAT CROPS automask.lua TOO. THAT ALWAYS MAINTAINS PERFECT ASPECT (NO SCALING).

options                 = { 
    auto                = true ,  --ALSO APPLIES TO auto_aspect.  IF REMOVED, A crop OCCURS @on_toggle & @playback-restart (BUT ~@UNPAUSE). 
    auto_delay          =    1 ,  --ORIGINALLY 4 SECONDS.  ALSO APPLIES TO aspect CONTROL.
    ...
    gsubs               = {                           --APPLY TO auto_aspect & script-message ARGUMENTS.  THESE EMULATE IN-SCRIPT ENVIRONMENTAL VARIABLES.
              ['$vida'] = '${video-params/aspect}'  , ['$osda'] = '${osd-dimensions/aspect}' ,  --$vida IS TRUE VIDEO-ASPECT.  $osda MEANS FULLSCREEN IF fs.
              ['$max' ] = '  max($vida,$osda)'      , ['$min' ] = '  min($vida,$osda)'       ,  
              ['$MAX' ] = '($max*1.5 )   '          , ['$MIN' ] = '($min/1.5 )   '           ,  --COULD ALSO BE NAMED $EXT & $INF (EXTREMUM & INFIMUM).
              ['$MID' ] = '($MAX*$MIN)^.5'          , ['$MEAN'] = '($MAX+$MIN)*.5'           ,  --$mid & $mean ALSO POSSIBLE.  
    },
    
    ----9 EXAMPLES OF auto_aspect CONTROL.  UNCOMMENT A LINE TO ACTIVATE IT.  APPLIES IF DYNAMIC ASPECT-CONTROL IS TOGGLED ON, BUT ONLY IF auto, @auto_delay, & NOT is1frame.  FORMULA MAY ALSO DEPEND ON aspect & Daspect FOR RANDOM-WALKS.  Daspect IS THE RATIO OF 2 PRIOR ASPECTS (MULTIPLICATIVE VELOCITY). 
       RANDOM_POWER        ; auto_aspect = '$MIN*($MAX/$MIN)^random()'      ,  --THIS FORMULA IS NON-LINEAR (POWER LAW).  A LARGE FLUCTUATION NEEDS LONGER auto_delay.  AN ULTRA-WIDE WINDOW RARELY EXCEEDS $osda (LIKE TOSSING HEADS 20 TIMES IN A ROW).  NON-LINEAR≈LINEAR. PROOF: ENTER  1.3*(1.8/1.3)^x  INTO A GRAPHICS-CALCULATOR (DESMOS.COM/calculator).
    -- VIDEO_ASPECT        ; auto_aspect = ''                               ,  --TRUE ASPECT.  SAME AS '$vida'.
    -- CYCLE_min_max       ; auto_aspect = 'aspect~=$max  and $max  or $min',  --CYCLES BTWN $min & $max.  AT LEAST ONE OF THESE IS THE TRUE aspect, ASSUMING NO crop.
    -- CYCLE_HDTV_TV       ; auto_aspect = 'aspect~=16/9  and 16/9  or 4/3 ',  --CYCLES BTWN HD & IPAD.
    -- RANDOM_N_FULLSCREEN ; auto_aspect = 'aspect~=$osda and $osda or $MIN*($MAX/$MIN)^random()'     ,  --CYCLES BTWN FULLSCREEN & RANDOM-POWER.  THIS ONE GIVES YOU FULLSCREEN HALF THE TIME, BUT I PREFER CUT DIAGONALS!
    -- UP_AND_ACROSS       ; auto_aspect = 'aspect==$MAX  and $osda or aspect==$MIN and $MAX  or $MIN',  --ANTI-CLOCKWISE ON TOP-RIGHT.  RIGHT-HANDERS MIGHT READ TOP-RIGHT.
    -- ACROSS_AND_DOWN     ; auto_aspect = 'aspect==$MAX  and $MIN  or aspect==$MIN and $osda or $MAX',  --     CLOCKWISE ON TOP-LEFT.
    -- LEFT_RIGHT_DOWN_UP  ; auto_aspect = 'aspect~=$osda and $osda or Daspect>1    and $MAX  or $MIN',  --INTERMEDIATE CYCLE. CYCLES BTWN $MIN & $MAX PASSING THROUGH WINDOW-SIZE $osda.  
    -- RANDOM_WALKER       ; auto_aspect = 'clip(aspect*Daspect*($MAX/$MIN)^(random()-.5),$MIN,$MAX)' ,  --SHOULD BE FASTER (auto_delay=.5), WHICH IS MORE VICIOUS.  MOMENTUM DEPENDS ON Daspect, WHICH TENDS TO KEEP IT GOING IN THE SAME DIRECTION.  SEEMS THE MOST DIFFICULT TO CODE WELL. DEPENDS ON EXISTING POSITION, MOMENTUM & RANDOM-POWER.
    -- LINEAR_DISTRIBUTION ; auto_aspect = '$MEAN+random()*((random(0,1)==0 and $MAX or $MIN)-$MEAN)' ,  --2 random NUMBERS ARE USED. (0,1) DETERMINES WHETHER TO ADD OR SUBTRACT (DIFFERENT GRADIENTS).
    ...
    framerate              = ''           ,  --DEFAULT=''=50 FPS.  REMOVE THIS TO REMOVE INTERPOLATION (''→nil).  INCOMPATIBLE WITH TRANSPARENCY (BUGFIX REMOVES IT).
...
o,p,m,timers                            = {},{},{},{}  --o,p,m=options,PROPERTIES,MEMORY  timers ={mute,aid,sid,playback_restarted,re_pause,apply_pad,apply_framerate,auto_delay}  playback_restarted BLOCKS THE PRIOR 3. 
android_surface_size,meta,gsubs,aspects = {},{},{},{}  --meta =METADATA FOR detect_crop.  aspects={OFF,restart} FOR PADDING (RELATIVE VALUES).  android_surface_size={w,h}.
abs,max,min,random                      = math.abs,math.max,math.min,math.random  --ABBREV.  m MEANS MEMORY NOT math.
...
function pexpand(arg)  --ALSO @print_arg, @show, @apply_pad, @apply_scale, @apply_crop & @detect_crop.  PROTECTED/PROPERTY EXPANSION.  '${speed}+2'=3. 
    if type(arg)~='string' then return arg end
    for     N    = 1,o.gsubs_passes do for pattern,repl in pairs(o.gsubs)  --gsubs MAY DEPEND EACH-OTHER. FULL EXPANSION REQUIRES $MIN, ETC.
        do  arg  = arg: gsub(pattern,repl) end end
...
-- for pattern,repl in pairs(o.gsubs) do for pattern2,repl2 in pairs(o.gsubs)  --o.gsubs_passes SHOULD BE MOVED HERE FROM pexpand.
    -- do  gsubs[pattern] = repl:    gsub(pattern2,repl2) end end
...
utils,label            = require 'mp.utils',mp.get_script_name()  --utils FOR SCRIPT-MESSAGING.  label=autocrop.
...
Daspect                = 1           --INITIAL DIVISION VALUE.  ASPECT_VELOCITY = Daspect/o.auto_delay.
...
function file_loaded()     --ALSO @event_handler & @property_handler.  
    ...
    if not detect_format then gp('msg-level')  --detect_format=nil MEANS ONCE ONLY, EVER.  @file-loaded BECAUSE OLD FFMPEG BUGS OUT SOONER.
        ...
        mp.set_property_native  ('msg-level',p['msg-level']) end     --h=oh MEANS NO STRETCHING (STUPID SCALER). OLD FFMPEG FAILS TO REPORT SELF-REFERENTIALITY (FALSE NEGATIVE). 
    ...
    for _,vf in pairs(gp('vf'))  --CHECK FOR @loop.  COULD DEFINE function is_filter_present.
    ...
    ----framerate  = fps                       DEFAULT=50             IS THE FINISH.  NULL-OP USES NO CPU.  FORCES alpha REMOVAL.  NEGATIVE TIME CAUSES BUG.  IT *CAN* OUTPUT VARIABLE fps BY COMBINING IT INSIDE GRAPH.
...
function apply_pad(pad_options,pixelformat,pad_scale_flags,par) --@script-message & @file_loaded.  PADDING APPENDS AFTER OTHER SCRIPT/S.
    ...
    pixelformat        =  pixelformat     or o.video_out_params.pixelformat or detect_format  --OVERRIDE  OR  DETECTOR.
    pad_scale_flags    =  pad_scale_flags or o.pad_scale_flags
    par                =  par             or o.video_out_params.par
    pad_time,framerate = gp('time-pos'),nil  --pad_time BLOCKS RE-PADDING TOO QUICKLY.  
    for _,vf in pairs(p.vf) 
    do framerate       = framerate or vf.label==label..'-framerate' end  --framerate HAS A BUG WHICH REMOVES alpha.
    insta_pause        = not p.pause and loop  --FOR PNG RELIABILITY.  CAUSES STUTTER ON FILM.
    
    mp.command((''
                ..(insta_pause and '   set pause yes;'            or '')                                       
                ..(framerate   and '%s vf  remove @%s-framerate;' or ''):format(command_prefix,label)
                ..                 '%s vf  append @%s-pad-scale:scale=flags=%s:eval=frame;' 
                ..                 "%s vf  append @%s-pad:lavfi=[format=%s,pad='%d:%d:%s',scale=%d:%d,setsar='%s'];"  --FINAL scale IS NULL-OP, UNLESS W OR H ODD. 
                ..(insta_pause and '   set pause no ;'            or '')
        ):format(command_prefix,label,pad_scale_flags,command_prefix,label,pixelformat,W2,H2,pad_options,W,H,par))
...
function apply_framerate()  --@apply_pad.  
    if o.framerate and v.id and not is1frame and not gp('video-out-params/alpha')  --~v.id ASSUMED TO BE TRANSPARENT.
    then mp.command(('%s vf append @%s-framerate:framerate=%s'):format(command_prefix,label,o.framerate)) end
end
...
function on_toggle()  --@script-message, @script-binding, & @property_handler.  
    ...
    on_toggle_aspect()  --TOGGLE BOTH crop & PADDING.  LIKE 2 SCRIPTS IN 1: BITE & STRETCH TECHNIQUE.
...
function apply_scale(aspect,toggle_duration,toggle_t_delay,toggle_expr)  --@script-message, @playback-restart, @property_handler, & @on_toggle_aspect.  SIMPLER TO SEPARATE UTILITY FROM ITS TOGGLE.  COULD BE RENAMED apply_aspect.  STRETCHES BLACK BARS WITHOUT CHANGING vo DSIZE (DISPLAY SIZE). 
    ...
    elseif auto then timers.auto_delay:kill  ()  --RESET AUTO ASPECT TIMER.  SMOOTHENS WINDOW RESIZING.
                     timers.auto_delay:resume() end 
    
    OFF2            = OFF2 and not aspect  --FLIP OFF2 @script-message.  RE-RUNNING AFTERWARDS DELETES aspect. FUTURE VERSION SHOULD MEMORIZE script-message aspect. 
    aspect          = pexpand  (aspect) or p['video-params/aspect']
    Daspect         = aspect/(m.aspect  or aspect)  --DIVISION VALUE FOR RANDOM WALKER
    m.aspect        = aspect
    ...
    aspectR         = aspects.OFF * (OFF2 and 1                                        --OFF VALUE.
                      or aspect / (p['osd-dimensions/aspect'] * o.osd_par_multiplier)) --TRUE ASPECT FORMULA.  IT'S JUST W/H*RATIO OF VIDEO:OSD.  IT'S RELATIVE TO W/H.  REAL ASPECT OF EACH PIXEL (osd-par) IS ASSUMED TO BE ACCOUNTED FOR IN osd-dimensions/aspect.  video-params/aspect ACCOUNTS FOR video-params/par (demux-par).  SHARED-MEMORY NEEDS FULLSCREEN BECAUSE video-params/aspect=osd-dimensions/aspect.  
    m.aspectR       = m.aspectR or aspectR  --INITIALIZE.
    
    m.pad_iw,pad_iw,m.pad_ih,pad_ih          = round(min(W2,H2*m.aspectR),2),round(min(W2,H2*aspectR),2),round(min(W2/m.aspectR,H2),2),round(min(W2/aspectR,H2),2)  --pad GRAPH WIDTHS & HEIGHTS: PRIOR(MEMORY)→TARGETS.  STORING THEM PROPERLY IN MEMORY IS MORE CODE, SO DEDUCE FROM m.aspect.
    m.aspectR,m.scale_time,m.toggle_duration = aspectR,scale_time,toggle_duration --MEMORY TRANSFER FOR RAPID TOGGLING.
    Dpad_iw,Dpad_ih                          = pad_iw-m.pad_iw,pad_ih-m.pad_ih    --DIFFERENCE VALUES.  Δ INVALID ON MPV.APP (NO-GREEK).  BOTH Dpad_i? VALUES MAY BE NON-0, WHICH IS THE BLANK/CUT-DIAGONAL METHOD (ASPECT CUTS ACROSS SCREEN).
    ...
    replace = is1frame    and (scale_w or scale_h)   --is1frame IS GLITCHING @WINDOW-RESIZE.
    Daspect =  (scale_w or scale_h) and Daspect or 1 --NO MOMENTUM IF NULL-OP.
    command = nil            
              or    replace and ('%s vf append @%s-pad-scale:scale=w=%d:h=%d:flags=%s:eval=frame;'):format(command_prefix,label,pad_iw,pad_ih,o.pad_scale_flags)  --MAINTAIN frame IN CASE OF SWITCH FROM MP4TAG TO MP4.
              or ''
...
function detect_crop(show_text)  --@script-message, @timers.auto_delay, @playback-restart & @on_toggle.
    ...
    aspect    = m.aspect     --INITIALIZE PRIOR aspect FOR RANDOM WALKER.  
    rescale   = (not p.pause --2 RESCALE POSSIBILITIES:  ~PAUSED OR AFTER FRAME-STEPPING THE auto_delay+.1.
                 or  scale_time and abs(p['time-pos']-scale_time)>=o.auto_delay+o.toggle_t_delay) 
                 and aspect     and auto   and not OFF2 and apply_scale(o.auto_aspect)  --ONLY IF auto & ON2. '' & ' ' BOTH CAST TO nil.  THIS SHOULD BE MOVED TO A NEW SCRIPT.
    ...
    crop         = is_effective and apply_crop(meta)
end
...
function   event_handler     (event)
    ...
    then is1frame_replaced  = is1frame
         Daspect,m.aspectR,m.aspect =  1,aspects.OFF, gp('osd-dimensions/aspect') * o.osd_par_multiplier  --EFFECTIVE VALUES.
         m.w,m.h,m.x,m.y,m.time_pos,m.scale_time,p.seeking = nil --FORCES vf-COMMANDS.
         rescale = not auto and apply_scale()         --INTERFERES WITH auto. 
         
         detect_crop()                          --AFTER seeking.  THIS CHOREOGRAPHS THE auto_aspect & automask, BUT IT GOES OFF WITHIN A MINUTE. INSTEAD OF SYNCING TO os.time, auto_aspect SHOULD SYNC TO time-pos!  THERE'S A FEW WAYS TO FIX THIS. AN astats FILTER CAN REPORT THE EXACT TICK OF FILM-TIME, OR A RESYNC LOOP COULD BE USED. LUA CAN EITHER SYNC TO REAL-TIME, OR FILM-TIME! A PERIODIC TIMER DOES NEITHER BY DEFAULT.
...
function property_handler(property,val)
    ...
    rescale                  =       (property=='osd-dimensions/aspect'     --2 RESCALES.  ~auto OR WOULD INTERFERE.  UNFORTUNATELY is1frame AUDIO GLITCHES.
                                 or   property=='vf')           and W2      and not OFF2 and not auto and apply_scale() 
...
for message,fn in pairs({loadstring=callstring,print=pprint,show=show,exit=exit,quit=cleanup,toggle=on_toggle,toggle_aspect=on_toggle_aspect,apply_limit=apply_limit,detect_crop=detect_crop,apply_crop=apply_crop,apply_pad=apply_pad,apply_aspect=apply_scale})  --SCRIPT CONTROLS.
...
----script-message             apply_aspect ${osd-dimensions/aspect}*(1+3*random())/2 .5                .12               sin(PI/2*(expr))^2         
----script-message             apply_pad    <pad_options>                             <pixelformat>     <pad_scale_flags> <par>
----script-message             apply_pad    x=(ow-iw)/2:y=(oh-ih)/2:color=WHITE@1     yuv420p           bicubic           1
