...
options={
    ...
    key_bindings          ='C c J j', --DEFAULT='C'. CASE SENSITIVE. DOESN'T WORK INSIDE SMPLAYER. TOGGLE DOESN'T AFFECT start & end TIME LIMITS.  J & j FOR DVORAK, BUT j IS ALSO next_subtitle (& IN VLC IS dec_audio_delay). 
    ...
    MAINTAIN_CENTER       ={0,0}, --{TOLERANCE_X,TOLERANCE_Y}. DOESN'T APPLY TO image. {0,0} MEANS NEVER MOVE THE CENTER (LIKE CROSSHAIRS). A SINGLE BLACK BAR ON 1 SIDE MAYBE OK (UNLESS is1frame). A TRIBAR FLAG WITH BLACK ON TOP OR BOTTOM NEEDS RATIO<1/3. MOVEMENTS IN CENTER TEND TO BE SPURIOUS (LIKE A DARK FLOOR).
    ...
    ----["path-substring"]={start,end,detect_limit}={SECONDS,SECONDS<0,number} (OR nil).  detect_limit IS FINAL OVERRIDE.
...
function start_file()  --ESTABLISH limits.
    ...
    limits={}
    for key,val in pairs(o.limits) do   key   =(key):lower()  --NOT CASE SENSITIVE.
        if     m.path:find(key,1,true) then limits={  -- 1,true = STARTING_INDEX,EXACT_MATCH  val ASSUMED TO BE table. 
                start       =val.start        or val[1],  
                ['end']     =val['end']       or val[2],
                detect_limit=val.detect_limit or val[3],  --FINAL OVERRIDE.
            } end end  
    if limits.start  and m.start =='none' then mp.set_property('start',limits.start ) end  --start MAY BE FASTER THAN seeking. TRIMMING TIMESTAMPS IS ANOTHER ALTERNATIVE. 
    if limits['end'] and m['end']=='none' then mp.set_property('end'  ,limits['end']) end 
end
...
function file_loaded() --ALSO @alpha & @vid.  THAT IS TRANSPARENCY & TRACK-ID.  alpha=nil ASSUMED @file-loaded.  MPV-v0.36 REQUIRES EXTRA .1s TO DETECT ALL video-params. 
    ...
    m,lavfi_complex  = {vid=v.id},mp.get_opt('lavfi-complex') 
    ...
    is1frame         =   v.image    and not lavfi_complex  --REQUIRE GRAPH REPLACEMENT IF image & ~complex. 
    ...
    ----scale     =w:h:...      :eval  DEFAULT=iw:ih:...:init  USING [vo] scale WOULD CAUSE EMBEDDED MPV TO SNAP @vid, SO USE display INSTEAD. EITHER WINDOW SNAPS IN, OR ELSE video SNAPS OUT.
    ...
    timers.auto_delay:resume()  --SMALL DELAY NEEDED FOR INITIAL DETECTION (is1frame).
end
...
function detect_crop()  --FOR RAW AUDIO TOO.  USUALLY LOOPS EVERY HALF A SECOND.
    if OFF or   not max_w or not auto and m.time_pos then return end --~auto ONLY CROPS ONCE & @on_toggle & @playback-restart.
...
function apply_crop(meta) 
    ...
    if is1frame then insta_pause=not pause  --GRAPH REPLACEMENTS. insta_pause DUE TO INTERFERENCE FROM OTHER SCRIPT/S.
        ...
        clip                = ('clip((t-%s)/(%s),0,1)'):format(time_pos,insta_unpause and 0 or o.toggle_duration)  --duration=0 ALSO VALID (insta_unpause).  A FUTURE VERSION COULD ALSO SUPPORT A BOUNCING OPTION (BOUNCING BLACK BARS).
        clip                = ('(1-cos(PI*%s))/2')     :format(clip)  --[0,1] DOMAIN & RANGE.  TIME EXPRESSION FOR SMOOTH-PADDING.  0=INITIAL, 1=FINAL.  A SINE WAVE PROGRESSION.  
...
function apply_pad()  --TARGETED scale COMMANDS pad EITHER HORIZONTALLY OR VERTICALLY.  
    if repeat_pad or m.pad_iw~=pad_iw then mp.command(('%s vf-command %s-pad width  %d+%d*%s %s'):format(command_prefix,label,m.pad_iw,pad_iw-m.pad_iw,clip,target_scale)) end  --CLIPS BTWN PRIOR pad_iw & NEXT pad_iw.
    if repeat_pad or m.pad_ih~=pad_ih then mp.command(('%s vf-command %s-pad height %d+%d*%s %s'):format(command_prefix,label,m.pad_ih,pad_ih-m.pad_ih,clip,target_scale)) end
end 
