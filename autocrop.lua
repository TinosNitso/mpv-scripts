...
----THIS VERSION HAS DOUBLE-mute TOGGLE, & MAINTAINS CENTER IN HORIZONTAL/VERTICAL, WITH TOLERANCE VALUES. ALSO SUPPORTS MULTIPLE DETECTORS, & bbox. vf-command EXPERIMENTAL. SMOOTHCROP CHOPPY, BUT UNIQUE TO THIS SCRIPT.
...
local options = {
    ...
    detect_limit    =30 , --ORIGINAL DEFAULT="24/255".  ONLY INTEGER COMPATIBLE WITH bbox (0→255). autocomplex OVERLAY MAY INTERFERE.
    ...
    TOLERANCE_RATIO  =.1 , --DEFAULT=.05. crop INSTANTLY IF COORDS CHANGE BY MORE THAN 5%, OR ELSE EVERY 60 SECONDS. SO A BIG crop IS INSTANT, BUT NOT LITTLE CROPS.
    ...
    CROPFRAMES=  8, --REMOVE FOR NO SMOOTHCROP. CROPS/CROP.   STILL EXPERIMENTAL (CAUSES OTHER SCRIPTS TO CHOP, BUT THE JOKE IS TOO GOOD). vf-command TOO CHOPPY. MAY NEED TO USE buffer.
    crop_delay=.01, --SECONDS. REMOVE FOR NO SMOOTHCROP. TEST WITH DOUBLE-mute.
    ...
    detectors ='cropdetect=%s:%s:1:0',  --MULTIPLE detectors. DEFAULT='cropdetect=limit=%s:round=%s:reset=1:0'. 'bbox=%s' IS AUTO-JPEG OVERRIDE.   %s=string SUBSTITUTION, FORMATTED @file-loaded. EXTRA cropdetect OR bbox DETECTORS MAYBE NEEDED FOR RELIABILITY, ALL IN THE autocrop GRAPH. reset>0 FOR vf_command. skip=0 FOR JPEG. 
...
for key,val in pairs({command_prefix='',detect_limits={},TOLERANCE_RATIO=.05,TOLERANCE_TIME=60,time_needed=0,scale={},detectors='cropdetect=limit=%s:round=%s:reset=1:0',key_bindings='C',toggle_on_double_mute=0,config={}})
...
function file_loaded(_,seeking) --OVERLOADS FOR vid TRACK CHANGE - INITIAL SEEK. TRACK CHANGE IS LIKE A NEW FILE-LOAD.
    ...
    if mp.get_property_bool('current-tracks/video/image') then o.detectors='bbox=%s'  --JPEG OVERRIDE. bbox WORKS BETTER THAN cropdetect (BUGGY). IMAGES MAY BE JPG, PNG, BMP, MP3. GIF IS not image. NO WEBP. TIFF TOP LAYER ONLY. 
        if mp.get_property('lavfi-complex')=='' then unpause = not mp.get_property_bool('pause')     --crop ONLY WORKS ON AN image IF IT'S PAUSED OR LOOPING (autocomplex). unpause RETURNS pause STATE. complex IS FASTER, BUT CAN'T CHANGE TRACK LIKE THIS METHOD.
    ...
    ----scale     =width:height DEFAULT=iw:ih   MULTIPLES OF 4 AVOID SNAPPING. (TEST ODD NUMBERED JPEGS.)   USING [vo] scale WOULD CAUSE SNAPPING ON vid CHANGE, SO I PREFER display.
...
function apply_crop(meta) 
    if meta then CROPFRAME=1   --meta MEANS NEW crop.
    ...
    if unpause then mp.command('no-osd set pause yes') end  --EACH crop SHOULD BE PAUSED IN THIS CASE.
    ...
    if CROPFRAME<o.CROPFRAMES then timers.SMOOTHCROP:resume()   --NEXT SMOOTHCROP. WORKS WITH RAW JPEG (NO EXTRA FILTERS!).
    elseif mp.get_property_bool('current-tracks/video/image') then o.auto=false    --0% CPU USAGE OVERRIDE FOR JPEG, AFTER FINAL SMOOTHCROP. DISABLE FURTHER DETECTIONS.
        timers.auto_delay:kill()
        mp.unobserve_property(detect_crop)
        if unpause then mp.command('no-osd set pause no') end end
end
...
----COMMENT SECTION. FORMATTING FOR cropdetect ACTUALLY REDUCES PERFORMANCE OF SMOOTHCROP.
