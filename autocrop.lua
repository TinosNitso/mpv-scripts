...
options={
    ...
    toggle_duration        =         .4 ,  --SECONDS TO TOGGLE PADDING. REMOVE FOR INSTA-TOGGLE.  STRETCHES OUT BLACK BARS, IF PLAYING.  SMOOTH-PADDING IS EASY, BUT NOT SMOOTH-CROPPING.  FOR JPEG, seek FORWARD TO RE-pad.  SMPLAYER.APP HAS TO GIVE MPV ITS OWN WINDOW (OR ELSE window-id=nil SO NO SIZING).  
    ...
    vf_command_t_delay     =         .12,  --DEFAULT=0 SECONDS.  RAPID TOGGLING HAS ~.1s LAG DUE TO A FEW FRAMES WHICH AREN'T REDRAWN FAST ENOUGH.  IT TAKES NEARLY AS LONG AS SENDING MESSAGES TO THE OPPOSITE SIDE OF THE EARTH!  BACKWARDS-seeking ADDS .5s.
    ...
    USE_INNER_RECTANGLE    =          true,  --AGGRESSIVE CROPPING. BOTH cropdetect & bbox GENERATE UNNECESSARY x1 x2 y1 y2 NUMBERS, WHICH ENABLE THE INNER RECTANGLE CALCULATION.  MAYBE FOR STATISTICAL REASONS.  COMPUTE x1,x2 = max(x1,x2-w,x),min(x2,x1+w,x+w) ETC BY SYMMETRY, THEN SHRINK w TO THE NEW x2-x1. 
    ...
    -- apply_min_ratio     =          true,  --UNCOMMENT TO crop ALL THE WAY DOWN TO detect_min_ratio. CAN BE SPURIOUS.  DEFAULT NULLIFIES is_excessive.
    ...
    -- toggle_expr         = '(1-cos(PI*%s))/2',  --DEFAULT=%s=LINEAR_IN_TIME_EXPRESSION  UNCOMMENT FOR NON-LINEAR SINUSOIDAL TRANSITION BTWN aspect RATIOS (HALF-WAVE). DOMAIN & RANGE BOTH [0,1].  A SINE WAVE IS 57% FASTER @MAX-SPEED, FOR SAME DURATION (PI/2=1.57). BUT ACCELERATION MAY BE A BAD THING!
    -- dimensions          =    {w=1680,h=1050},  --DEFAULT={w=display-width,h=display-height}  THESE ARE OUTPUT OVERRIDES.  EMBEDDED MPV MAY HAVE display-width=nil @file-loaded, SO OVERRIDE IS REQUIRED. 
    options                = {
...
aspects             = {}                      --{ON,OFF,restart,osd,video} FOR PADDING.  ON & OFF ARE RELATIVE VALUES.
...
function property_handler(property,val)
    p[property] = val
    meta        = {max_w=p.width,max_h=p.height} --FORCE min_* RECOMPUTATIONS.  METADATA FOR detect_crop INITIALIZED HERE.
    aspects.OFF =   nil        --FORCE       RECOMPUTATION .  NEW WINDOW SIZE.  aspects.restart REMAINS ORIGINAL VALUE.
    reload      = v and ( nil  --v MEANS LOADED.  4 RELOAD CONDITIONS: @NEW-vo, @is1frame, @alpha & @image.
                    or  property=='video-params'               and val    and (not W or is1frame or val.alpha and not p['video-params/alpha'] and not v.image and v.id) --NEW vo, OR Δalbumart, OR TRY SWITCH TO TRANSPARENCY.  is1frame MUST BE RE-DRAWN.  image & no-vid ALREADY COUNT AS TRANSPARENT. TRANSPARENCY TAKES TIME TO DETECT, BUT DELAYED TRIGGERS ARE BAD.
    ...
    re_pad      =       property=='osd-dimensions/aspect'      and W2     and OFF2 and not is1frame     and apply_pad()                          --CHANGING WINDOW SIZE!  OFF2 ACTIVATES apply_pad.  is1frame AUDIO WOULD GLITCH, SO MUST TOGGLE.  ALSO FAILS WHEN PAUSED (MUST UNPAUSE OR FRAME-STEP). 
    load_pad    =       W                                      and not W2 and reload_pad() --PADDING LOADS HERE, ONCE PER file.  W2 (EVEN) MEANS LOADED PADDING.  W=nil MAY OCCUR FOR --no-vid + lavfi-complex.
    if property~='path' then return end           --path_handler BELOW. COMBINES BOTH start-file & end-file(~path).
    v,W,W2,limits = nil                           --CLEAR SWITCHES.
...
function file_loaded()  --ALSO @property_handler.
    ...
    if not (p.width or v['demux-w']) then mp.msg.warn("autocrop only works for videos.") --THIS VERSION ALSO SETS start & end FOR AUDIO.  return CONDITIONS REQUIRE EITHER PARAMATERS OR A TRACK.  width ALONE CAUSES OCCASIONAL TRIGGER-LAG ON WINDOWS. SOME LAG CAN CAUSE MORE LAG.
        insta_pause       = insta_pause and mp.set_property_bool('pause',nil) and nil       --UNPAUSE FOR AUDIO.
        return end        
    insta_pause           = insta_pause or not p.pause                                                           --REPAUSE ON RELOAD.
    W                     = o.dimensions.w or o.dimensions[1] or p['display-width' ] or p.width  or v['demux-w'] --OVERRIDE  OR  display  OR  VIDEO-DIMENSIONS.  width SOMETIMES BUGS OUT @file-loaded.
    H                     = o.dimensions.h or o.dimensions[2] or p['display-height'] or p.height or v['demux-h']
    ...
    loop                  = v.image               and p['lavfi-complex']=='' and not no_mask --ALSO FOR is1frame, FOR SIMPLICITY.
    ...
    ----pad        = w:h:x:y:color @reload_pad DEFAULT=0:0:0:0:BLACK  0 MAY MEAN iw OR ih. FOR TOGGLE OFF. RETURNS EXTRA BLACK BARS TO CORRECT aspect. MPV HAS A WINDOW TO COLOR IN.
...
function reload_pad()  --@property_handler & @seek.  DELAYED TRIGGER BECAUSE MPV REQUIRES EXTRA ~.1s TO DETECT osd-dimensions/aspect>0, & PADDING APPENDS AFTER OTHER SCRIPT/S.  
    aspects         = { osd=p['osd-dimensions/aspect'] or 0 , video=(p['video-params'] or {}).aspect }
    if aspects.osd == 0 or not aspects.video then return end               --aspects.osd=0 BEFORE MPV MEASURES ITS OWN WINDOW (OCCASIONAL STARTING LAG), BUT CAN BE nil FOR vo=shm.  aspects.video=nil DURING complex TRACK-CHANGES.  ALSO @seek.  
    W2,H2           = round(W,2),round(H,2)                                --EVENS ONLY FOR PADDING.  W2 ALSO ACTS AS PADDING-LOADED SWITCH.
    aspects.ON      = W2/H2
    aspects.OFF     = aspects.video * aspects.ON / aspects.osd             --TRUE ASPECT FORMULA. MULTIPLY INPUT BY RATIO OF display:osd ASPECTS.
    ...
    m.video_aspect,m.osd_aspect = aspects.video,aspects.osd
...
function on_seek()
    if not p['video-params'] then return end  --nil@lavfi-complex
    ...
    replace_pad        = not (loop or  aspects.osd==p['osd-dimensions/aspect'] and m.video_aspect==p['video-params'].aspect) and reload_pad()  --IF WINDOW/video CHANGES SIZE, @%s-pad-scale SHOULD BE REPLACED, SINCE IT'S DEFAULT OFF STATE IS NOW DIFFERENT.  BUT loop MESSES UP THE TIMESTAMPS (OFF).
end
...
function on_toggle()  --@key_binding & @property_handler.  
    ...
    return true     --SIMPLIFIES double_mute.
end
...
function apply_pad(aspect)  --@playback-restart, @property_handler, @on_toggle_pad & @on_toggle.  SIMPLER TO SEPARATE UTILITY FROM ITS TOGGLE.  STRETCHES BLACK BARS WITHOUT CHANGING [vo] scale. 
    ...
    if not (aspects.video and p['time-pos']) then return end                              --time-pos=nil WHEN SKIPPING 10 MINS PASSED end-file, BUT playback-restart TRIGGERS.  aspects.video=nil DURING complex TRACK-CHANGES.
    aspects.OFF     = aspects.OFF or aspects.video * aspects.ON / p['osd-dimensions/aspect'] --TRUE ASPECT FORMULA.  IT'S JUST WHATEVER THE INPUT IS, MULTIPLIED BY RATIO OF STARTING TO OSD ASPECT, BECAUSE OF HOW THE GRAPHS ARE SET.  THERE ARE ALSO OTHER EXPLANATION/S.  REAL ASPECT OF EACH PIXEL (osd-par) IS ASSUMED TO BE ACCOUNTED FOR IN osd-dimensions/aspect. demux-par IS ACCOUNTED FOR IN video-params/aspect.
...
function apply_crop(meta)  --@detect_crop & @on_toggle.
    ...
    if not (p.width and p['time-pos']) then return end  --width=nil IF TOGGLING vo.
...
function detect_crop()  --@timers.auto_delay, @playback-restart & @on_toggle.
    ...
    show_text = o.meta_osd and mp.command(('show-text                    "'
                    ...
                    ..'    min_w,min_h      = %s,%s                   \n\n'  --480,270
                    ..'vf-metadata/%s = \n${vf-metadata/%s}              "'            
                ):format(_VERSION,pad_iw,pad_ih,W2,H2,meta.max_w,meta.max_h,meta.min_w or '',meta.min_h or '',label,label))  --nil INVALID ON MPV.APP.  min_* RESET @property_handler.
