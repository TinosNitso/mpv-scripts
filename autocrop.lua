...
function apply_crop(meta) 
    ...
    if is1frame 
    then mp.command(('%s vf pre @%s-pad:lavfi=[scale=%d:%d:eval=frame,pad=%d:%d:(ow-iw)/2:(oh-ih)/2:%s:frame,setsar=1]'):format(command_prefix,label,w_target,h_target,W,H,o.pad_color))  --GRAPH REPLACEMENTS.  apply_pad HERE.
         mp.command(('%s vf pre @%s:lavfi=[%s,crop=%d:%d:%d:%d:1:1]'):format(command_prefix,label,detector,meta.w,meta.h,meta.x,meta.y))  --detector NEEDED FOR TOGGLE.
    else if not target
        ...
        for whxy in ('w h x y'):gmatch('[^ ]') do if m[whxy]~=meta[whxy]  --EXCESSIVE vf-command CAUSES LAG, SO CHECK FIRST.
...
----A FUTURE VERSION COULD CROP WITHOUT CHANGING ASPECT (EQUIVALENT TO AUTO-zoom). o.MAINTAIN_ASPECT? WITH WIDE-SCREEN A PORTRAIT COULD BE TALLER, BUT NOT ULTRA-FAT.
