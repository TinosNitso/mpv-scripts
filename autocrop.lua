...
----THIS VERSION HAS DOUBLE-mute TOGGLE, & MAINTAINS CENTER IN HORIZONTAL/VERTICAL, WITH TOLERANCE VALUES. ALSO SUPPORTS MULTIPLE DETECTORS, & bbox. IT'S BASED ON THE OFFICIAL autocrop.lua
...
local options = {
    ...
    detect_limit    =  32,  --ORIGINAL DEFAULT="24/255".  BASED ON 8-BITS (0→255).
    ...
    detect_limits ={   --NOT CASE SENSITIVE. detect_limit MAY VARY WITH media-title IF THE FOLLOWING SUBSTRINGS ARE FOUND.
                    ...
                    [                       'L.A.D.Y G.A.G.A']=100, --THIS autocrop HAS 10 HOUR CROPS WITH 0% CPU USAGE, WITH NO GLITCH @file-loaded.
                   },
    ...
    TOLERANCE_RATIO  =.01,  --DEFAULT 0. crop INSTANTLY IF COORDS CHANGE BY MORE THAN 1%, OR ELSE EVERY 60 SECONDS. SO A BIG crop IS INSTANT, BUT NOT LITTLE CROPS.
    TOLERANCE_TIME   =60,   --DEFAULT 60. IRRELEVANT IF TOLERANCE_RATIO=0.
    ...
    -- CROPFRAMES=8,crop_delay=.03, --DEFAULTS 1,1. CROPS/CROP FOR SMOOTHCROP, & SECONDS TO WAIT BTWN SUCCESSIVE CROPS FOR SMOOTH CROPPING. 0.03 MEANS UP TO 33 CROPS PER SECOND.      vf-command IS TOO CHOPPY, SO CURRENTLY HAVE TO RE-PLACE crop FOR EACH FRAME, BUT THAT'S CAUSING LAG. MAY NEED TO USE buffer.
    ...
    format  ='yuv422p',     --DEFAULT='yuv422p' (HALF COLOR-WIDTH)  yuv422p yuv440p yuv444p ARE GOOD, BUT yuv420p rgb24 ETC DON'T ALWAYS WORK WITH cropdetect.  SHOULD MATCH OTHER SCRIPTS FOR SMOOTH TOGGLE. 
    ...
    key_bindings         ='C c J j',--DEFAULT='C'. CASE SENSITIVE. J IS DVORAK FOR C (CROP), BUT IS ALSO next_subtitle (& IN VLC IS dec_audio_delay). THESE DON'T WORK INSIDE SMPLAYER. THE REMOVAL & RENEWAL OF CROPPING IS SMOOTH.
...
for key,val in pairs({command_prefix='',detect_limits={},TOLERANCE_RATIO=0,TOLERANCE_TIME=60,time_needed=0,format='yuv422p',scale={},detectors='cropdetect=limit=%s:round=%s:reset=1:0',key_bindings='C',toggle_on_double_mute=0,config={}})
...
if not o.crop_delay or not o.CROPFRAMES or o.crop_delay<=0 or o.CROPFRAMES<=1
then o.crop_delay,o.CROPFRAMES = 1,1 end --DEFAULT: NO SMOOTH CROPPING.

local m={width=o.scale[1], height=o.scale[2]} --crop METADATA scale OVERRIDE.
if m.width and m.height then mp.command(('%s vf pre @%s:lavfi=[format=%s,scale=%s:%s,setsar]'):format(o.command_prefix,label,o.format,m.width,m.height)) end  --OPTIONAL scale OVERRIDE: STOPS EMBEDDED MPV SNAPPING. scale,format,setsar DEFINE THE SNAP GRAPH (SAME LABEL).
...
function file_loaded(_,seeking) --OVERLOADS FOR vid TRACK CHANGE - INITIAL SEEK. TRACK CHANGE IS LIKE A NEW FILE-LOAD.
    ...
    lavfi=('format=%s,%s,crop=%%s:%%s:%%s:%%s,scale=%d:%d,setsar')
        :format(o.format,o.detectors,          m.width,m.height    ) 
    ...
    ----setsar     IS THE FINISH. MACOS BUGFIX. ALONG WITH scale & format, STOPS MPV SNAPPING ITS OWN WINDOW.
    
    
    mp.command(('%s vf pre @%s:lavfi=[%s]'):format(o.command_prefix,label,lavfi)   --LINUX .AppImage MAY FAIL IF GRAPH IS INSERTED BEFORE file-loaded. MUST DELAY, BUT THIS CAN ALSO CAUSE EMBEDDED MPV TO SNAP IF format OR SAR CHANGES. DEFAULT GRAPH INSERTION, FOR cropdetect.
                                           :format('iw','ih',0,0))
...
function apply_crop(meta) 
    ...
    if o.vf_command then for key in ('x y out_w out_h'):gmatch('%g+') do mp.command(('%s vf-command %s %s %s'):format(o.command_prefix,label,key,m[key])) end --vf-command NOT WORKING. x y FIRST, BY TRIAL & ERROR. IT MAY NEED MULTIPLES OF 4 ONLY.
    else mp.command(('%s vf pre @%s:lavfi=[%s]'):format(o.command_prefix,label,lavfi)  --Apply crop. GRAPH REPLACEMENT. SAFER OPTION. SANDWICHED BTWN complex & automask.
