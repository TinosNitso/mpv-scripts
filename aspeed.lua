...
options={  --ALL OPTIONAL & MAY BE REMOVED.
    ...
    mpv                      = {  --REMOVE FOR NO CHILDREN OVERRIDE (clocks+filterchain ONLY). LIST ALL POSSIBLE mpv COMMANDS, IN ORDER OF PREFERENCE.  FIRST MATCH SPAWNS ALL CHILDREN.  A COMMAND MAY NOT BE A PATH.  HAVE NEVER TESTED ANDROID.
    ...
    suppress_osd       =    true, --REMOVE FOR speed_expr OSD.  APPLIES ALSO TO CHILDREN.
    -- speed_expr      = 'clip(1+.2*${percent-pos}/60,1,1.2)',  --UNCOMMENT FOR DYNAMIC speed CONTROL, set EVERY HALF-SECOND.  THIS CLIPS speed BTWN 1 & 1.2, ACHIEVED AFTER 60% OF DURATION. speed+1% EVERY FEW SECONDS.
    -- meta_osd        =    true, --UNCOMMENT FOR audio STATISTICS (astats METADATA).  SHOULD BE REMOVED IN FUTURE VERSION, SINCE MPV-v0.37+ SYNC PROPERLY WITHOUT IT.
    ...
    options_children = {
        ...
        'terminal no','msg-level all=error','priority abovenormal',  --DEFAULTS yes,all=status,normal.  BY DEFAULT SMPLAYER LOGS ALL speed CHANGES VIA terminal.  priority ONLY VALID ON WINDOWS.  
    },
...
for  opt,val in pairs({key_bindings='',double_mute_timeout=0,extra_devices_index_list={},filterchain='anull',mpv={},timeouts={},max_random_percent=0,max_speed_ratio=1.2,seek_limit=1,auto_delay=.5,resync_delay=60,os_sync_delay=.01,mutelr='mutel',options={},options_children={},clocks={},})
...
function property_handler(property,val) --ALSO @timers.auto  CONTROLLER WRITES TO txtfile, & CHILDREN READ FROM IT.  ONLY EVER pcall, FOR RELIABLE INSTANT write & SIMULTANEOUS io.remove.
    ...
    a_id              = p['current-tracks/audio/id']     --aid=false WHEN LOCKED. a_id MORE RELIABLE!
    ...
    then double_mute  = property=='mute' and loaded and (timers.mute:is_enabled() and on_toggle() or timers.mute:resume())  --DOUBLE-mute TOGGLE.  SMPLAYER DOUBLE-MUTE WHILE seeking MAY FAIL (CANCELS ITSELF OUT).
        speed         = o.speed_expr and loadstring('return '..mp.command_native({'expand-text',o.speed_expr}))()
        set_speed     =   speed      and speed~=p.speed    and mp.command(command_prefix..' set speed '..speed)
        ...
        if not mpv or a_id and not property    and txt.speed>0 then return end --return CONDITIONS.  OVERRIDE: NO CHILDREN.  OR NOT STARTED YET.  OR ELSE IT'S THE auto IDLER, TO STOP timeout WHEN txt.speed=0 (UNLESS JPEG). THE IDLER SHOULD ALWAYS BE RUNNING FOR RELIABILITY.
        meta_osd      = o.meta_osd and not OFF and samples_time and mp.osd_message(mp.get_property_osd('af-metadata/'..label):gsub('\n','    '))  --TAB EACH STAT (TOO MANY LINES), FOR osd.  samples_time CORRESPONDS TO NEW OBSERVATION.
        ...
        txtfile:write( ('%s\n%s\n%s\n%s\n%s\n%s'):format( --CONTROLLER REPORT.  SECURITY PRECAUTION: NO ARBITRARY COMMANDS OR SETS (property NAMES). A set COULD HOOK AN UNSAFE EXECUTABLE, SIMILAR TO PIPING TO A SOCKET. DIFFERENT LINES MIGHT REQUIRE SECURITY OVERRIDES.
            ...
            a_id      or 'no'   ,  --no BEFORE YOUTUBE LOADS.  OFTEN aid=no BUT a_id=1.  lavfi-complex COMBINES MULTIPLE STREAMS.  
            (txt.mute or p.mute or not a_id) and 0 or p.volume or 0,  --OFF-SWITCH & mute.  "or 0" @load-script.  AUDIO STREAM ITSELF MAY CYCLE ON & OFF WITH A KEYBIND, WITH SMOOTH PLAYBACK.
            txt.speed           ,
            ...
            ----FUTURE VERSION SHOULD HAVE ANOTHER LINE FOR FIRST-CHILD FEEDBACK.  CHILDREN TOO SLOW TO seek THROUGH LONG YOUTUBE VIDEOS, SO CONTROLLER READS txtfile & UNMUTES. 
        ))
    ...
    mp.msg.error('FEEDBACK'..time_pos)
end
for property in ('mute pause seeking audio-params/samplerate current-tracks/audio/id frame-drop-count speed volume path af-metadata/'..label):gmatch('[^ ]+')  --BOOLEANS, NUMBERS, string & table.  INSTANT write TO txtfile. CASCADE @volume REQUIRES pcall.  samplerate MAY DEPEND ON lavfi-complex.  
