...
if mutelr then math.randomseed(pid)  --UNIQUE randomseed FOR ALL subprocesses. OTHERWISE TEMPO MAY BE PREDICTABLE OR SAME.
    ...
    for _,command in pairs(o.mpv) do if mp.command_native({'subprocess',command}).error~='init' then mpv=command  --error=init IF INCORRECT COMMAND. OTHERWISE error USUALLY killed, BUT ALWAYS RETURNED. subprocess RETURNS, NOT run.
            break end end 
...
function start_file()  --LAUNCH INSTANTLY, FOR YOUTUBE. AT LEAST 4 STAGES: LOAD-SCRIPT start-file file-loaded playback-restart
    ...
    for N,device in pairs(devices) do for mutelr in ('mutel muter'):gmatch('[^ ]+') do if not (N==1 and mutelr==o.mutelr) --DON'T LAUNCH ON PRIMARY device CHANNEL.  'mutel muter' FOR SIMPLICITY BECAUSE IF LOOPING OVER ALL CHANNELS THEIR GEOMETRY IS UNKNOWN.
            then mp.command_native({name='subprocess',playback_only=false,capture_stdout=false,capture_stderr=false,detach=true,  --EXTRA FLAGS FOR UNIX RELIABILITY. COMPLICATED COMMANDS MAY REQUIRE NAMED KEYS.
...
function file_loaded() 
    mp.command(("no-osd af pre '@%s:lavfi=[%s]'"):format(label,lavfi):format(map))   --GRAPH ONLY EVER INSERTS HERE. astats USES SOURCE samplerate.  "''" FOR SPACEBARS IN filterchain.
end
...
function playback_restart() --ALSO @frame-drop-count
    if path then mp.command(('af-command %s map %d'):format(label,map))  --LIKE automask COVERS SPECIAL CASE OF TOGGLE DURING seeking, BUT AFTER seek.
...
function on_toggle(property)  --CONTROLLER ONLY. INSTA-TOGGLE (SWITCH), NOT PROPER FULL toggle. (PERHAPS A DOUBLE-TAP key_bind COULD TRIGGER FULL TOGGLE.)  subprocesses MAINTAIN SYNC WHEN OFF.
    ...
    elseif property and not timers.mute:is_enabled() then timers.mute:resume() --START TIMER OR ELSE TOGGLE.  DOUBLE-MUTE MAY FAIL TO OBSERVE EITHER mute IF seeking.
        ...
        if OFF then mp.add_timeout(.5,function() txt.mute=OFF end)  --DELAYED MUTE ON, OR ELSE LEFT CHANNEL CUTS OUT A TINY BIT.  txtfile IS TOO QUICK FOR af-command! THE EXACT timeout COULD BE A NEW option. GRAPH REPLACEMENT INTERRUPTS PLAYBACK.
...
function property_handler(property,val)     --CONTROLLER WRITES TO txtfile, & subprocesses READ FROM IT.  ONLY EVER pcall, FOR RELIABLE INSTANT write/SIMULTANEOUS io.remove.
    ...
    txtfile:close()  --OPTIONAL: MAYBE NEEDED FOR RELIABILITY.
    ...
    if txt.speed==0 then mp.set_property_number('volume',0) end
...
mp.observe_property('audio-params/samplerate','number',function(   _,val) samplerate,initial_time_pos = val,nil end) --samplerate DEPENDS ON lavfi-complex.  ALSO RESET SAMPLE COUNT? 
