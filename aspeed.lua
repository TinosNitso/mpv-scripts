...
options                      = {
    ...
    speed                    =      '${speed}'                             ,  --EXPRESSION FOR DYNAMIC speed CONTROL, set EVERY HALF-SECOND.  CAN USE ANY MPV PROPERTY (LIKE ${percent-pos}) IN ANY LUA FORMULA.  '${speed}' IS A NULL-OP.  TOGGLES OFF & ON WITH DOUBLE-mute!
    -- speed                 =      '${speed}<1.2 and ${speed}+.01 or 1'   ,  --UNCOMMENT TO CYCLE speed BTWN 1 & 1.2.  PRESS BACKSPACE TO RESET BACK TO 1.  REPLACE 1.2 & 1 FOR DIFFERENT BOUNDS. THESE TAKE 10s TO FULLY CYCLE.
    -- speed                 = 'clip(${speed}+math.random(-1,1)/100,1,1.2)',  --UNCOMMENT TO RANDOMIZE VIDEO speed, USING A BOUNDED RANDOM WALK.  -1%,0%,1% EVERY HALF-SECOND.
    ...
    meta_osd           =   false,  --true FOR audio STATISTICS (astats METADATA).  CONTROLLER ONLY.  SHOULD BE REMOVED IN FUTURE VERSION. MPV-v0.37+ SYNC PROPERLY WITHOUT IT. (IT WAS NECESSARY FOR OLD MPV.)
    ...
    options_children = {
        ...
        '     sid  no       ','keep-open yes        ','geometry 25%',  --no SUBTITLE-ID.  keep-open IS BECAUSE NO RELOADING IS ALLOWED.  geometry LIMITS CHILDREN.
...
----    STYLE CODES: \\,N,an#,fs#,bord#,c######,fscx## = \,NEWLINE,ALIGNMENT-NUMPAD,FONT-SIZE(p),BORDER(p),COLOR,FONTSCALEX(%)  (DEFAULT an0=an7=TOPLEFT)    MORE: alpha##,b1,shad#,be1,i1,u1,s1,fn*,fr##,fscy## = TRANSPARENCY,BOLD,SHADOW(p),BLUREDGES,ITALIC,UNDERLINE,STRIKEOUT,FONTNAME,FONTROTATION(°ANTI-CLOCKWISE),FONTSCALEY(%)  EXAMPLES: USE {\\alpha80} FOR TRANSPARENCY. USE {\\fscx130} FOR +30% IN HORIZONTAL.  A TRANSPARENT clock CAN BE BIGGER. be ACTS LIKE SEMI-BOLD.  
    ...
    child_params = '{N=0,pid}',  --DECLARATION OF PARAMETERS FED TO CHILDREN.  N=1 (FIRST-CHILD) PROVIDES FEEDBACK.  PARENT PROCESS-ID DETERMINES txtfile THE CHILDREN READ FROM.
}
...
is_controller  = not o.child_params.pid  --CONTROLLER HAS N=0 BUT NO PARENT-pid.  
...
clocks,abdays,LOCALES,txt = {},{},{},{}  --LOCALES IS LIST OF SUB-TABLES, FOR LOTE. NEVER USED BY CHILDREN (UNLESS THEY ALSO HAVE A clock).  txt FOR property_handler.
...
then clock             = clocks[1] and mp.create_osd_overlay('ass-events')  --ass-events IS THE ONLY VALID OPTION.  AT LEAST 1 CLOCK OR nil.  COULD ALSO SET res_x & res_y FOR BETTER THAN 720p FONT QUALITY.
    ...
    for N,audio_device in pairs(mpv and devices or {}) do for mutelr in ('mutel muter'):gmatch('[^ ]+')   --ONLY IF mpv.  DON'T LAUNCH ON PRIMARY device CHANNEL. 
        do script_opts = (N>1 or mutelr~=o.mutelr) and ('%s-mutelr=%s,%s-child_params="{N=%d,pid=%d}",%s'):format(label,mutelr,label,N,p.pid,p['script-opts']) --PREFIX label-.  FIRST-BORN IS SPECIAL.  mutelr & audio-device VARY.  CHILDREN WITH 2 MOUTHS MAY BE MUTED LEFT OR RIGHT. BUT INTERNAL PC SPEAKERS SHOULD COUNT AS ONLY 1.
...
function file_loaded()  --ALSO @seek.
    loaded,a_id = true,mp.get_property_number('current-tracks/audio/id')
    if not a_id then return end
    m.map = OFF and 0 or 1  --0,1 = OFF,ON  graph SWITCH @INSERTION.  FUTURE VERSION SHOULD ALSO CHECK IF m.map=map ALREADY.
...
function apply_astreamselect(map)  --@playback-restart, @on_toggle & @property_handler.
    if not a_id then return end
    map = map or OFF and 0 or 1
    mp.command(('af-command %s map %d %s'):format(label,map,target or ''))  --target ACQUIRED @samples_time.
end
...
function property_handler(property,val) --ALSO @timers.auto  CONTROLLER WRITES TO txtfile, & CHILDREN READ FROM IT.  ONLY EVER pcall, FOR RELIABLE INSTANT write & SIMULTANEOUS io.remove.
    ...
    if not mp2os_time                                                                 --3 return CONDITIONS.  1) AWAIT SYNC.  
        or not is_controller and     property and property~='af-metadata/'..label     --CHILD OBSERVATION ENDS HERE, EXCEPT ON astats TRIGGER. CONTROLLER PROCEEDS.
        or     is_controller and not property and a_id and not (p.pause or p.seeking) --CONTROLLER IDLER  ENDS HERE, UNLESS PAUSED/seeking OR JPEG. IT DOES write OBSERVATIONS.  seeking SAVES YOUTUBE LOAD.  A COMMON ISSUE WITH MANY DIRECT LINKS TO 1 function IS DOUBLE-NEGATIVE RETURNS.
    then return end 
    
    mp_time,p['time-pos'] = mp.get_time(),mp.get_property_number('time-pos') 
    os_time,set_speed,write = mp2os_time+mp_time  --os_time=TIMEFROM1970  PRECISE TO 10ms.
    ...
    txtfile           = io.open(txtpath)  --'r' MODE. 'r+' ALSO WORKS.  ALTERNATIVE io.lines RAISES ERROR.  CONTROLLER READS TOO, FOR FEEDBACK. 
    if txtfile        
    ...
    if is_controller 
    then osd_message = samples_time   and not OFF and o.meta_osd and mp.osd_message(mp.get_property_osd('af-metadata/'..label):gsub('\n','    ')) --TAB EACH STAT (TOO MANY LINES).  samples_time CORRESPONDS TO EVERY HALF-SECOND.
        speed        = samples_time   and not OFF and loadstring('return '..mp.command_native({'expand-text',o.speed}))() or p.speed                     --samples_time AVOIDS INFINITE CASCADE @speed.
        set_speed    = p.speed~=speed 
        un_mutelr    = not p.seeking and txt.seeking=='yes' and apply_astreamselect(0)  --BUGFIX FOR SLOW CHILD. (UNMUTE WHILE IT SEEKS.)
        mutelr       = not OFF       and txt.seeking=='no'  and apply_astreamselect(1) 
        write        = mpv                                                --CONTROLLER REPORT (IF mpv).  SECURITY PRECAUTION: NO ARBITRARY COMMANDS OR SETS (property NAMES). A set COULD HOOK AN UNSAFE EXECUTABLE, SIMILAR TO PIPING TO A SOCKET. DIFFERENT LINES MIGHT REQUIRE SECURITY OVERRIDES.
        txt.path     = p.path    or ''                                    --BLANK @load-script.
        txt.aid      = a_id      or 'no'                                  --no BEFORE YOUTUBE LOADS.
        ...
        txt.speed    = p.pause                          and 0 or   speed
        txt.os_time  = round(os_time      ,.001)  --MILLISECOND PRECISION.
        txt.time_pos = round(p['time-pos'],.001)
        txt.seeking  = p.seeking and 'yes' or 'no' end  --IS FIRST-BORN CHILD seeking? IF SO, SHOULD UN-mutelr.  LONG YOUTUBE VIDEOS CAN BE SLOW TO LOAD A SPECIFIC CHILD-STREAM.
        ----FUTURE VERSION MIGHT HAVE ANOTHER LINE FOR SMOOTH TOGGLE (SMOOTH-MUTE USING t-DEPENDENT af-command).
    if o.child_params.N==1 and p.seeking and txt.seeking=='no' then write,txt.seeking = true,'yes' end
    
    if   write 
    then write   = ''
        for line in ('path aid volume speed os_time time_pos seeking'):gmatch('[^ ]+') 
        do write = write..txt[line]..'\n' end
        txtfile  = io.open(txtpath,'w')           --w+=ERASE+WRITE ALSO WORKS.  MPV.APP REQUIRES txtfile BE WELL-DEFINED.
        txtfile:write(write)  
    ...
    set_pause       = (time_from_write>o.timeouts.pause or not txtfile) and not is_controller  --EITHER CONTROLLER STOPPED OR FILE INACCESSIBLE.  THIS MAY TRIP ACCIDENTALLY?
    command         =  --quit OR pause.  SOMETIMES txtpath IS INACCESSIBLE, SO AWAIT timeout.  txtfile DOESN'T ORDER A quit BECAUSE A timeout IS STILL NEEDED ANYWAY, FOR THE OCCASIONAL FAILURE TO READ "quit" FAST ENOUGH.
                          set_speed                      and  command_prefix..' set speed '..speed --CONTROLLER TITULAR COMMAND.
                      or  time_from_write>o.timeouts.quit                  and 'quit'   
                      or  set_pause                                        and 'set pause yes'     
    command         = command and mp.command(command)
    if is_controller or not (txtfile and txt.path) then return end --CHILDREN BELOW, UNLESS BLANK txtfile (EITHER pause OR NOTHING).
    target_pos      = txt.time_pos+time_from_write*txt.speed       --=Δtime_pos=Δos_time*speed
    time_gained     = p['time-pos']-target_pos  
    seek            = math.abs(time_gained)>o.seek_limit and (target~='' or samples_time)  --NEW MPV OR ELSE ACCURATE samples_time.
    time_gained     = seek         and 0 or time_gained  --seek→(time_gained=0)
    speed           = (time_from_write>o.timeouts.pause or txt.aid=='no' or txt.seeking=='yes') and 0 --0 MEANS pause.  seeking→pause MIGHT FIX A YOUTUBE STARTING GLITCH.  CHILDREN ALWAYS START PAUSED.
    ...
    set_speed       = speed>0 and (target~='' or samples_time)  --NEW MPV OR ELSE ACCURATE samples_time.
...
----THIS IS LIKE 2 SCRIPTS IN 1.  HOWEVER clock.lua COULD ALSO INCLUDE AN ALARM.  RESYNCING THE EXACT TICK EVERY MINUTE USES 0% CPU.  BOTH INDIA & CHINA ARE CHARGED.  COLOMBIA MIGHT BE POSSIBLE ({\\fscy200}%I{\\fscy100}).  MISSISSIPPI STATE FLAG IS CHARGED (1 MISSISSIPPI | 2 MISSISSIPPI | 3 MISSISSIPPI).  AN UKRAINIAN/POLISH STYLE MIGHT REQUIRE SPLITTING THE OSD IN HALF (ACROSS), TO COLOR IT.  CONCEIVABLY ADVERTISEMENTS COULD FIT INSIDE EACH DIGIT OF A CLOCK.
