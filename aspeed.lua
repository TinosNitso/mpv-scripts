...
options                      = {
    ...
    clocks       = {  --TOGGLE LINES TO INCLUDE/EXCLUDE VARIOUS STYLES FROM THE LIST.  REPETITION VALID.  CLOCKS REQUIRE VIDEO OR image.  A SIMPLE LIST OF STRINGS IS EASY TO RE-ORDER & DUPLICATE, LIKE REPEATING Yemen FOR THE ARABIC.
        duration = 2, --INTEGER SECONDS.  0/nil MEANS NO CLOCK.  TIME PER CLOCK STYLE THROUGHOUT CYCLE.  STYLE TICKS OVER EVERY SECOND SECOND. (ON THE DOUBLE.)
        offset   = 0, --INTEGER SECONDS.  CHANGE STYLE ON EVENS OR ODDS? 0=EVEN.  ALL SMPLAYER INSTANCES HAVE SAME CLOCK @SAME TIME.
...
----    "        COUNTRY             HOUR     MINUTE   SECOND  POPULATION  [    AbDays            '-'=HALF-SPACE ]  {\\STYLE OVERRIDES}              ◙              ↓↓(CLOCK SIZE)                 %DIRECTIVES              ",  --"{" REQUIRED, & EVERYTHING BEFORE IT IS REMOVED.  {} ALONE REMOVES LEADING 0 FOLLOWING IT.  AbDays (ABBREVIATED DAYS) LOCALES START WITH Sun (BUT Mon IN REALITY), & CAN BE REPLACED WITH ANYTHING (1 LOTE CAN BE COPIED OVER ALL THE OTHERS).  https://lh.2xlibre.net/locales/ FOR LOCALES, BUT GOOGLE TRANSLATE ALSO.  
        ...
        "NETHERLANDS  NEDERLAND      RED    ◙ WHITE  ◙   BLUE   18M  [  Zon  -Ma-  -Di-   -Wo-  -Do-  -Vr-  -Za- ]  {\\an3\\c     0\\fs37\\bord0}%a\\N{\\c251DAD\\fs55\\bord3}%I{\\cFFFFFF     }\\N%M{\\c85471E       }\\N%S",  --PARAGUAY & CROATIA SIMILAR BUT CHARGED.  YUGOSLAVIA WAS CHARGED REVERSE.  Zon MAY HAVE 3 LETTERS?
        ...
        "     RUSSIA  РОССИЯ         WHITE  ◙  BLUE  ◙    RED  147M  [ -Вс-  -Пн-  -Вт-   -Ср-  -Чт-  -Пт-  -Сб- ]  {\\an3\\c     0\\fs37\\bord0}%a\\N{\\cFFFFFF\\fs55\\bord3}%I{\\cA73600     }\\N%M{\\c1827D6       }\\N%S",  --SLOVENIA SIMILAR, BUT CHARGED. SERBIA IS CHARGED REVERSE.  THE COLORS DICTATE THE MEANING OF THE LETTERS. 
        ...
        "    ESTONIA  EESTI          BLUE   ◙ BLACK  ◙  WHITE    1M  [--P-- --E-- --T--  --K-- --N-- --R-- --L-- ]  {\\an3\\c     0\\fs37\\bord0}%a\\N{\\cCE7200\\fs55\\bord3}%I{\\c   0\\bord1}\\N%M{\\cFFFFFF\\bord3}\\N%S",  --P=SUNDAY BUT IT'S ALSO LIKE PM IN LATIN.  
...
o,p,m,timers = options,{},{},{}      --p=PROPERTIES  m={map,path}=MEMORY  timers={mute,auto,os_sync,osd}
...
function apply_astreamselect(map)  --@playback-restart, @on_toggle & @property_handler.
    map       = map or (N>0 or not OFF and mpv) and 1 or 0  --DEDUCE INTENDED map.  ONLY EVER CHANGES FOR CONTROLLER.  
...
function os_sync()  --@property_handler & @playback-restart.  RUN 10ms LOOP UNTIL SYSTEM CLOCK TICKS. os.time() HAS 1s PRECISION WHICH MAY BE IMPROVED TO 10ms, TO SYNC CHILDREN. 
    ...
    if sync_time>time1 then mp2os_time,time1 = sync_time-mp.get_time(),nil  --mp2os_time=os_time_relative_to_mp_clock  IS THE CONSTANT TO ADD TO MPV CLOCK TO GET TIMEFROM1970 TO WITHIN 10ms.  WARNING: os.clock WORKS EQUALLY WELL ON WINDOWS, BUT NOT UNIX+VIRTUALBOX (CPU TIME DIFFERENT).  mp.get_time()=os.clock()+CONSTANT  (WITHIN HALF A MILLISECOND.)
        timers.os_sync:kill()
...
function property_handler(property,val) --ALSO @timers.auto  CONTROLLER WRITES TO txtfile, & CHILDREN READ FROM IT.  ONLY EVER pcall, FOR RELIABLE INSTANT write & SIMULTANEOUS io.remove.
    ...
    mp_time,p['time-pos'],set_speed,write = mp.get_time(),mp.get_property_number('time-pos'),nil  --",nil" RARELY REQUIRED.
    ...
    then osd_message  = samples_time     and not OFF and o.metadata_osd and mp.osd_message(mp.get_property_osd('af-metadata/'..label):gsub('\n','    ')) --TAB EACH STAT (TOO MANY LINES).  samples_time CORRESPONDS TO EVERY HALF-SECOND.
        double_mute   = property=='mute' and   m.map and (not timers.mute:is_enabled() and (timers.mute:resume() or 1) or on_toggle())  --m.map AWAITS file-loaded.  SMPLAYER DOUBLE-MUTE WHILE seeking MAY FAIL (CANCELS ITSELF OUT).
        ...
        un_mutelr     = not p.seeking    and txt.seeking=='yes' and apply_astreamselect(0)  --BUGFIX FOR SLOW FIRST-CHILD. un_mutelr WHILE IT SEEKS.  LONG YOUTUBE VIDEOS TEND TO GLITCH.
        mutelr        = mpv and not OFF  and txt.seeking=='no'  and apply_astreamselect(1)  --INVALID WITHOUT CHILDREN.
        ...
        txt.os_time   = round(os_time      ,.001)  --ms PRECISION.
        txt.time_pos  = round(p['time-pos'],.001) 
        txt.seeking   = txt.seeking or 'no' end --INITIALIZED AS no. THIS LINE IS FULLY CONTROLLED BY FIRST-CHILD. 
    seeking           = p.seeking  and 'yes' or 'no'
    if   N           == 1 then write,txt.seeking = txt.seeking~=seeking,seeking end  --COULD ALSO PROVIDE FEEDBACK @shutdown, TOO (OPTIONAL).
    if   write          --FIRST-CHILD WRITES, TOO. 
    then write        = ''
