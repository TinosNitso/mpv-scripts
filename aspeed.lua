...
options={ --ALL OPTIONAL & MAY BE REMOVED.
    ...
    -- title_clock_only=true,   --OVERRIDE: NO audio EFFECT, EXCEPT FOR FILTERS.   THIS option MEANS THE clock DOESN'T HAVE TO BE COPY/PASTED INTO OTHER SCRIPT/S (THIS SCRIPT CONTROLS IT). A video CLOCK MAY BE FANCY & DANCE AROUND, BUT NOT THIS ONE.
    ...
    -- mutelr='muter',           --DEFAULT='mutel'  UNCOMMENT TO SWITCH PRIMARY CONTROLLER CHANNEL TO LEFT. PRIMARY device HAS 1 CHANNEL IN PERFECT SYNC TO video. STANDARD HARDWARE USUALLY HAS A PRIMARY, BUT IT'S 50/50 (HEADPHONES OPPOSITE TO SPEAKERS).
    ...
    time_needed  =  5, --DEFAULT=5   SECONDS. NO RANDOMIZATION WITHIN 5 SECS OF file-loaded & end-file (STRONG FINISH). ALSO WAITS THIS LONG FOR astats TO STABILIZE SAMPLE COUNT.
...
for key,val in pairs({title_duration=5,title='',clock='',FILTERS='anull',extra_devices_index_list={},mutelr='mutel',start=.7,DELAY=.5,max_random_percent=0,seek_limit=2,resync_delay=30,os_sync_delay=.01,time_needed=5,timeout=10,key_bindings='',toggle_on_double_mute=0,config={}})
...
----dynaudnorm   =...:p:m:...:c  →s64  DEFAULTS .95:10:0:0 (PEAK TARGET [0,1] : MAX GAIN [1,100] : CORRECTION (DC,0Hz)) INSERTS AFTER astats BECAUSE IT SLIGHTLY CHANGES SAMPLE COUNT OVER 10 HOURS (NORMALIZER MAY NOT BE DETERMINISTIC).  ALTERNATIVE NORMALIZERS (loudnorm & acompressor) ARE MORE DEFECTIVE IN MY EXPERIENCE.
----anull         PLACEHOLDER
...
----astreamselect=inputs:map  ENABLES INSTA-TOGGLE. "af-command" NOT "af toggle". ANY ATTEMPT TO DOUBLE CHANGE GRAPHS OR DO A FULL TOGGLE CAUSES CONTROLLER GLITCH (CAN VERIFY BY TOGGLING acompressor IN SMPLAYER). SHOULD BE PLACED LAST BECAUSE SOME FILTERS (dynaudnorm) DON'T ACTUALLY KNOW WHICH STREAM TO FILTER, BECAUSE THAT'S DETERMINED BY REMOTE CONTROL (af-command 0 OR 1).
...
mp.register_event('shutdown'   ,function() os.remove(txtpath) end)  --ALTERNATIVE event=end-file       NO RECYCLE BIN. audio INSTANCES quit.
...
function on_toggle(mute)   --TOGGLE clock & audio RANDOMIZATION. THE IDEAL INSTA-TOGGLE DOES ALMOST NOTHING, UNLIKE FULL TOGGLE. (PERHAPS A DOUBLE-TAP key_bind COULD TRIGGER FULL TOGGLE.)
    if mute=='mute' and not timers.mute:is_enabled() then timers.mute:resume() --START TIMER OR ELSE TOGGLE.
        return end
    
    OFF,initial_time_pos,command = not OFF,nil,'af-command '..label..' map '   --OFF=MEMORY FOR SWITCH. FORCE initial_time_pos RESET.
    if not OFF then command=command..1     --TOGGLE ON     LUA SUPPORTS string..number
    else            command=command..0 end --TOGGLE OFF
    mp.observe_property('seeking','bool',function() mp.command(command) end)  --TRIGGERS INSTANTLY, & ON seeking BECAUSE THAT RESETS GRAPH STATE. UNLIKE automask, DOESN'T REQUIRE PAUSED frame-step.
    clock_update() 
end
...
function set_speed(property)    --property='af-metadata/aspeed'     THIS IS THE MAIN FUNCTION. CONTROLLER WRITES TO txtfile, OR audio INSTANCES READ FROM IT. SHOULD ONLY EVER BE PCALLED IN CASE OF SUDDEN STOP (SIMULTANEOUS io.remove & io.write).
    paused,time_remaining = mp.get_property_bool('pause'),mp.get_property_number('time-remaining')  --meta=astatsMETADATA
    if not paused and not property then return end    --DO NOTHING IF PLAYING BUT NOT property. auto timer ONLY EXISTS FOR WHEN paused (NO astats UPDATE).
    
    time_pos,TIMEFROM1970,samples_time = mp.get_property_number('time-pos'),os.time(),mp.get_property_native('af-metadata/'..label)['lavfi.astats.Overall.Number_of_samples']/44100  --samplerate TIME = #SAMPLES/SR         time-pos, playback-time & audio-pts WORK WELL OVER 1 MINUTE, BUT NOT 1 HOUR. AUTO-SYNC MAY WORK IN MPV-V36, BUT NOT V35. SO USE astats samples COUNT.  
    ...
    speed=1-time_gained/.5    --time_gained→0 OVER THE NEXT .5 SECONDS (astats UPDATE TIME)
    if time_remaining>o.time_needed and samples_time>o.time_needed     --DON'T RANDOMIZE BEFORE samples_time STABILIZES, NOR NEAR end-file. 
...
mp.observe_property('af-metadata/'..label,'native',function(property) pcall(set_speed,property) end)   --TRIGGERS EVERY HALF A SECOND. pcall SIMPLIFIES set_speed CODE.
...
----SERIOUS  BUG: STARTUP WITH EXCESSIVE LAG ALONG WITH autocomplex. IN THAT CASE MUST STOP & PLAY (RESET).     IT COULD BE DUE TO asplit IN lavfi-complex.
