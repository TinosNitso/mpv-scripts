...
options={  --ALL OPTIONAL & MAY BE REMOVED.
    ...
    options            = {
        ' image-display-duration inf  ',  --DEFAULT=1  FOR JPEG clock.
    ...
    options_subprocesses={  --APPLY TO ALL NON-PRIMARY CHANNELS.
        '       vid no  ','  terminal no     ', --DEFAULTS auto,yes  REMOVE TO SEE THE RANDOMIZED VIDEO OF EACH CHANNEL.
        ...
        '  geometry 25% ',' msg-level all=no ','ytdl-format bestaudio/best',  --geometry IF vid yes.  all=no STOPS CONTROLLER LOG FROM FILLING UP. SOMEHOW SUBPROCESSES PIPE BACK, IN WINDOWS. 
    ...
    clocks       = {     --TOGGLE LINES TO INCLUDE/EXCLUDE VARIOUS STYLES FROM THE LIST.  REPETITION VALID.  CLOCKS REQUIRE VIDEO.  A SIMPLE LIST OF STRINGS IS EASY TO RE-ORDER & DUPLICATE, LIKE REPEATING YEMEN FOR THE ARABIC.
        ...
        -- no_locales          = true, --UNCOMMENT FOR English ONLY.  REPLACE [AbDays] WITH [ABDAYS] FOR UPPERCASE, OR [abdays] FOR LOWERCASE. EACH CLOCK CAN DICTATE ITS OWN AbDays. ◙=\\N FOR VERTICAL SPELLING (Sun→S◙u◙n).
...
----    "         COUNTRY             HOUR     MINUTE   SECOND  POPULATION       [AbDays]            {\\STYLE OVERRIDES} %DIRECTIVES                                                                        ",  --{ REQUIRED, & EVERYTHING BEFORE IT IS REMOVED.  {} ALONE REMOVES LEADING 0 FOLLOWING IT.  AbDays LOCALES START WITH Sun (BUT Mon IN REALITY), & CAN BE REPLACED WITH ANYTHING (1 LOTE CAN BE COPIED OVER ALL THE OTHERS).  https://lh.2xlibre.net/locales/ FOR ABBREVIATED DAYS IN ALL LOCALES, ETC.  
        ...
        "          NIGERIA            GREEN    WHITE    GREEN   231M  [Sun,Mon,Tue,Wed,Thu,Fri,Sat]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c  8000\\fs55\\bord3}%I{\\cFFFFFF       } %M{\\c  8000         } %S",  --BICOLOR TRIBAND.  THE ONLY ENGLISH PLAIN TRIBAND.  WHITE ALWAYS IN THE MIDDLE. ORDER ALIGNS WHITES & REDS.  
        " IVORY COAST  CÔTE D'IVOIRE  ORANGE   WHITE    GREEN    31M  [Dim,Lun,Mar,Mer,Jeu,Ven,Sam]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c  82FF\\fs55\\bord3}%I{\\cFFFFFF       } %M{\\c449A00         } %S", 
        ...
        "     AUSTRIA  ÖSTERREICH     RED    ◙ WHITE  ◙ RED       9M  [So ,Mo ,Di ,Mi ,Do ,Fr ,Sa ]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c2E10C8\\fs55\\bord3}%I{\\cFFFFFF     }\\N%M{\\c2E10C8       }\\N%S",  --BICOLOR. HORIZONTAL TRICOLOR (VERTICAL TAB).  LIKE A TAB FROM THE FLAG.  BLACK Day MAY OVERLAP WITH BLACK BARS ON SCREEN-EDGE.  {\\fr-90} IS ANOTHER OPTION.
        ...
        "       YEMEN  اليمن           RED    ◙ WHITE  ◙ BLACK    34M   [  ح,  ن,  ث,  ر ,  خ,  ج,  س]        {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c2611CE\\fs55\\bord3}%I{\\cFFFFFF     }\\N%M{\\c     0\\bord1}\\N%S",  --AbDays BACK-TO-FRONT.  REPRESENTS ARABIA. HOWEVER BOTH INDIA & CHINA ARE CHARGED.  A PAIR OF RTL (RIGHT-TO-LEFT) CHARS MIGHT LOOK BETTER FOR THIS CODE.
        "          BOLIVIA            RED    ◙ YELLOW ◙ GREEN    12M  [Do ,Lu ,Ma ,Mi ,Ju ,Vi ,Sa ]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c1C29DA\\fs55\\bord3}%I{\\c  E4F4     }\\N%M{\\c337A00       }\\N%S",
        ...
        "    BULGARIA  БЪЛГАРИЯ       WHITE  ◙ GREEN  ◙ RED       6M  [Вс ,Пн ,Вт ,Ср ,Чт ,Пт ,Сб ]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\cFFFFFF\\fs55\\bord3}%I{\\c  9900     }\\N%M{\\c    CC       }\\N%S",
        ...
        "     ESTONIA  EESTI          BLUE   ◙ BLACK  ◙ WHITE     1M  [P  ,E  ,T  ,K  ,N  ,R  ,L  ]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\cCE7200\\fs55\\bord3}%I{\\c   0\\bord1}\\N%M{\\cFFFFFF\\bord3}\\N%S",  --P,K = SUNDAY,WEDNESDAY  BUT ALSO FRIDAY,TUESDAY IN HUNGARIAN.  P IS ALSO LIKE PM IN LATIN.  THE COLORS DICTATE THE MEANING OF THE SYMBOLS. 
        "     GERMANY  DEUTSCHLAND    BLACK  ◙ RED    ◙ GOLD     85M  [So ,Mo ,Di ,Mi ,Do ,Fr ,Sa ]  {\\an3\\b1\\c0\\fs37\\bord0}%a\\N{\\c     0\\fs55\\bord1}%I{\\c  FF\\bord3}\\N%M{\\c  CCFF       }\\N%S",
...
for _,opt in pairs(is_controller and o.options or o.options_subprocesses)
...
function property_handler(property,val)  --CONTROLLER WRITES TO txtfile, & SUBPROCESSES READ FROM IT.  ONLY EVER pcall, FOR RELIABLE INSTANT write/SIMULTANEOUS io.remove.
    ...
    command    = time_from_write>o.timeouts.quit and 'quit' or (time_from_write>o.timeouts.pause or not txtfile) and 'set pause yes'  --SOMETIMES txtpath IS INACCESSIBLE, SO AWAIT timeout.  txtfile CAN'T TELL WHEN TO quit, BECAUSE SOME SUBPROCESSES WOULD quit WITHOUT THE OTHERS.
    ...
    for line in ('aid volume speed os_time time_pos priority'):gmatch('[^ ]+') 
    do txt[line]    = lines() end --LINES 2→7
    ...
    command         =                        '' 
        ..(               target_pos    and ('seek %s absolute exact;'):format(target_pos  ) or '')  --absolute MORE RELIABLE.  SYNC USING seek INSTEAD OF speed (BETTER TO SKIP THE TRACK THAN ACCELERATE ITS SPEED).  ANOTHER LINE OF CODE MAY BE NEEDED TO IMPROVE INITIAL DRUM-ROLL (FROM --idle TRIGGER).
        ...
        ..(p.priority ~=  txt.priority  and ('set priority %s       ;'):format(txt.priority) or '')  --BOTH nil ON UNIX. priority FOR WINDOWS.  PROPAGATING VIA TASK MANAGER NOT SUPPORTED BECAUSE MPV DOESN'T KNOW ITS OWN priority.
        ..(p.pause    ~=     (speed==0) and ('set pause    %s       ;'):format(txt.pause   ) or '')
        ..(samples_time and   speed> 0  and ('set speed    %s       ;'):format(speed       ) or '')  --REQUIRES NON-0 TARGET speed, & ACCURATE time_pos.
    if command        ~='' then mp.command(command) end
end
